type HazLenzKnowledgeMatchSectionProps = {
  safeScopeResult: any;
};
...
export default function HazLenzKnowledgeMatchSection({
  safeScopeResult,
}: HazLenzKnowledgeMatchSectionProps) {
    ...
    <details className="mt-4 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-sm">
      <summary className="flex cursor-pointer list-none items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs font-semibold leading-5 text-slate-700 dark:text-white">
            {matches.length
      ...
      <div className="mt-4 border-t border-slate-200 dark:border-slate-800 pt-4">
        <p className="text-sm font-semibold leading-6 text-slate-700 dark:text-white">
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="mt-1 text-xs font-bold text-slate-700 dark:text-white">
                ...
                {match.excerpt && (
                  <p className="mt-2 line-clamp-3 text-xs font-semibold leading-5 text-slate-700 dark:text-white">
                    {match.excerpt}
                  </p>
                )}
              </div>
        ...
        {knowledgeMatch.caution && (
          <p className="mt-3 text-[11px] font-semibold leading-5 text-slate-700 dark:text-white">
            {knowledgeMatch.caution}
          </p>
        )}
      </div>
