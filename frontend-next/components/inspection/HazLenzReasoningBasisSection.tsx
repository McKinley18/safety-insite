...
import HazLenzDrawer from "@/components/hazlenz/HazLenzDrawer";

type HazLenzReasoningBasisSectionProps = {
  safeScopeResult: any;
};
...
export default function HazLenzReasoningBasisSection({
  safeScopeResult,
}: HazLenzReasoningBasisSectionProps) {
      ...
      <div className="space-y-3">
        <div className="rounded-xl bg-slate-50 dark:bg-slate-950 px-3 py-3">
          ...
          <p className="mt-1 text-sm font-black text-slate-900 dark:text-slate-100">
            {formatValue(basis?.primaryReasoningSource || "hazlenz_governed_brain")}
          </p>
        ...
        {basis?.explanation && (
          <p className="rounded-xl bg-blue-50 px-3 py-3 text-xs font-semibold leading-5 text-slate-800 dark:text-slate-200">
            {basis.explanation}
          </p>
        )}
        ...
        {!!hierarchy?.primaryBasis?.length && (
          <div>
            ...
            <ul className="mt-2 list-disc space-y-1 pl-5 text-sm font-semibold leading-6 text-slate-800 dark:text-slate-200">
