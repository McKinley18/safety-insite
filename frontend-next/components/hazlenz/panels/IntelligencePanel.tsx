...
import { HazLenzDisplayAdapter, DisplaySection } from '../../../lib/hazlenzTypes/adapters/intelligence-display.adapter';
import { ReviewerFeedbackPanel } from './feedback/ReviewerFeedbackPanel';
import { AppButton } from '../../ui/AppButton';

function renderHazLenzValue(value: any): React.ReactNode {
  ...
  if (Array.isArray(value)) {
    const items = value
      .map((item) => renderHazLenzValue(item))
      .filter(Boolean);
  ...
  if (typeof value === "object") {
    const entries = Object.entries(value)
      .map(([key, item]) => {
        const rendered = renderHazLenzValue(item);
...
export const HazLenzSection = ({ section }: { section: DisplaySection }) => {
  if (!section.isVisible) return null;

  const rendered = renderHazLenzValue(section.content);
  if (!rendered) return null;
...
export const HazLenzIntelligencePanel = ({ adapter }: { adapter: HazLenzDisplayAdapter }) => {
  const [showFeedback, setShowFeedback] = useState(false);

  return (
    <div className="rounded-2xl bg-slate-50 dark:bg-slate-950 p-4">
      <HazLenzSection section={adapter.summary} />
      <HazLenzSection section={adapter.scenario} />
      <HazLenzSection section={adapter.evidence} />
      <HazLenzSection section={adapter.standards} />
      <HazLenzSection section={adapter.risk} />
      <HazLenzSection section={adapter.correctiveActions} />
      <HazLenzSection section={adapter.guardrails} />
