...
import { downloadHazLenzBrainBundle } from "./hazlenzBrainBundle";
...
function getHazLenzAuthHeaders() {
...
function runBasicHazLenzFallback(text: string, payload: any) {
...
function buildOfflineHazLenzFallback(input: {
  text: string;
  scopes?: string[];
...
export async function runHazLenzClassify(input: {
  text: string;
  scopes?: string[];
  evidenceTexts?: string[];
  ...
  const headers = getHazLenzAuthHeaders();
...
export async function sendHazLenzFeedback(payload: any) {
...
export async function runHazLenzOffline(input: {
  ...
  try {
    const fallbackResult = buildOfflineHazLenzFallback({
      text: input.observationText,
      scopes: ["all"],
    });

    return {
      engine: "hazlenz_reasoning_orchestrator_v1",
