...
function validateContent(text: string, part: string, subpart: string): boolean {
  ...
  return text.length > 500 && hasCfrMarker;
}

export class OshaEcfConnector {
  constructor(
    private readonly sourceKey: string,
    private readonly listFilename: string,
  ) {}
  ...
  async discover(): Promise<OshaDiscoveryItem[]> {
    ...
    const metadata = buildSourceRegistryMetadata(this.sourceKey);
    ...
    for (const item of items) {
      try {
        ...
        results.push({
          externalId: `osha-ecfr-${slugFromUrl(item.url)}`,
          title: item.titleHint || "OSHA eCFR Standard",
          sourceUrl: item.url,
