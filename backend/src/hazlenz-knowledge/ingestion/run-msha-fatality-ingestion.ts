...
async function main() {
  ...
  const run = await runRepo.save(
    runRepo.create({
      ...
      metadataJson: {
        ...
        splitSourceTypes: ["fatality_report", "fatality_alert"],
      },
    }),
  );
  ...
  for (const item of discovered) {
    const isFatalityAlert = item.sourceUrl.includes("/fatality-alert");
    const effectiveSourceType = isFatalityAlert ? "fatality_alert" : "fatality_report";
    const citationPrefix = isFatalityAlert
      ? "MSHA-FATALITY-ALERT"
      : "MSHA-FATALITY-REPORT";

    const citation = `${citationPrefix}-${item.externalId}`.toUpperCase();
    ...
    document.sourceType = effectiveSourceType as any;
    ...
    await chunkRepo.save(
      chunkRepo.create({
        ...
        chunkIndex: 0,
        sectionHeading: isFatalityAlert
          ? "Fatality alert prevention pattern"
          : "Full fatality report learning pattern",
