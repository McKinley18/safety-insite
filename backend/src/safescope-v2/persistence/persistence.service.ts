...
export class SafeScopePersistenceService {
  private readonly useFileFallback = true; // For local/dev if DB not ready
  ...
  async save(record: Omit<SafeScopeAuditRecord, 'id' | 'createdAt' | 'updatedAt'>): Promise<SafeScopeAuditRecord> {
    if (this.repository && !this.useFileFallback) {
      const entity = this.repository.create(record as DeepPartial<SafeScopeAuditRecordEntity>);
  ...
  async find(filter: AuditRecordFilter): Promise<SafeScopeAuditRecord[]> {
    if (this.repository && !this.useFileFallback) {
      const query = this.repository.createQueryBuilder('record');
      if (filter.type) query.andWhere('record.type = :type', { type: filter.type });
      if (filter.workspaceId) query.andWhere('record.workspaceId = :workspaceId', { workspaceId: filter.workspaceId });
  ...
  async getById(id: string): Promise<SafeScopeAuditRecord | undefined> {
    if (this.repository && !this.useFileFallback) {
      const entity = await this.repository.findOne({ where: { id } as any });
      return entity ? this.mapEntityToType(entity) : undefined;
    }
  ...
  async updateStatus(id: string, status: string, metadataUpdate: Record<string, any> = {}): Promise<SafeScopeAuditRecord | undefined> {
    if (this.repository && !this.useFileFallback) {
      const entity = await this.repository.findOne({ where: { id } as any });
      if (entity) {
