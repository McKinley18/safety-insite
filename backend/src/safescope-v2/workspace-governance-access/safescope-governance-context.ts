...
import { SafeScopeRole, UserGovernanceContext } from './workspace-governance.types';
 ...
 * `SafescopeV2Controller` and was therefore reachable only by requests that arrived at that
 * controller. §262 adds a second server-side caller: the authoritative Expert execution path runs
 * the deterministic analysis itself, and `SafescopeV2Service.classify` consults this context to
 ...
 */
const ROLE_MAP: Record<string, SafeScopeRole> = {
  'ORG_OWNER': 'owner',
  'OWNER': 'owner',
  'SUPER_ADMIN': 'admin',
  'ADMIN': 'admin',
...
export function resolveSafeScopeGovernanceContext(user: any): UserGovernanceContext {
