import { HazLenzCausalRiskOutput } from './causal-risk.types';
...
export class CausalRiskService {
  evaluate(input: CausalRiskInput): HazLenzCausalRiskOutput {
  ...
  async analyzeCausalRisk(observationUnderstanding: any, fusedText: string): Promise<HazLenzCausalRiskOutput> {
  ...
  private inferConfidence(input: {
    ...
    text: string;
  }): HazLenzCausalRiskOutput['confidence'] {
    let score = 0.2;
