...
import { HAZLENZ_CURATED_STANDARDS } from '../standards/hazlenz-standards.data';
...
function governedCitationKeys(): Set<string> {
  ...
  for (const record of HAZLENZ_CURATED_STANDARDS) keys.add(key(String(record.citation || '')));
  return keys;
}
