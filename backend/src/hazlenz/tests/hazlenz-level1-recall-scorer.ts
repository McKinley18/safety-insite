...
// Level-1 authority — the same `HazLenzService.classify()` the product
...
//
//   npx ts-node src/hazlenz/tests/hazlenz-level1-recall-scorer.ts \
...
import { HazLenzIntelligenceOrchestrator } from '../orchestration/intelligence-orchestrator.service';
...
async function makeService() {
  ...
  const ds = new DataSource({
    ...
    database: databaseUrl ? undefined : process.env.DB_DATABASE || process.env.DB_NAME || 'hazlenz',
    entities: [Standard],
  ...
  const service = new HazLenzService(
    ...
    new HazLenzIntelligenceOrchestrator(),
