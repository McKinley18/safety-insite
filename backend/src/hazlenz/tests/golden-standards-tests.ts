 ...
 * `synchronize: true` and defaulting to the `hazlenz` development database when
 * DATABASE_URL was unset -- so `npm run test:hazlenz-standards` with no explicit
...
function assertReadTargetIsIntentional() {
  ...
  const dbName = databaseUrl
    ...
    : process.env.DB_NAME || process.env.DB_DATABASE || 'hazlenz';
...
const ds = new DataSource({
  ...
  database: databaseUrl ? undefined : process.env.DB_NAME || 'hazlenz',
  entities: [Standard],
  synchronize: false,
});
...
async function run() {
  ...
  console.log('');
  console.log(`HazLenz standards tests: ${passed} passed, ${failed} failed`);
