...
import { HazLenzReasoningSnapshot } from './reasoning-snapshot.entity';

@Injectable()
export class ReasoningSnapshotService {
  constructor(
    @InjectRepository(HazLenzReasoningSnapshot)
    private readonly snapshotRepo: Repository<HazLenzReasoningSnapshot>,
  ) {}
  ...
  assertSnapshotAccess(
    snapshot: HazLenzReasoningSnapshot | null,
    user: any,
  ): void {
    if (!snapshot) {
      return;
  ...
  buildSnapshot(input: {
    ...
    intelligence?: any;
  }): Partial<HazLenzReasoningSnapshot> {
  ...
  buildSnapshotSummary(snapshot: HazLenzReasoningSnapshot | null) {
    if (!snapshot) {
      return null;
    }
