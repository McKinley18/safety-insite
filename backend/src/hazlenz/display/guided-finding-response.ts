...
function standardRecords(response: any) {
  return [
    ...
    // corpus-hydrated by HazLenzService.hydrateFindingScopedStandards(); it goes FIRST so an
...
export function attachGuidedFindingResponse(response: any, request: ClassifyDto) {
  ...
  const primaryStandard = citation ? {
    ...
    // adapter, at hazlenz.controller.ts:319). The three wire values are unchanged.
