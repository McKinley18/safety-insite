const DISPLAY_REPLACEMENTS: Array<[RegExp, string]> = [
  [/\bHazLenz Intelligence Orchestrator\b/g, "HazLenz AI Orchestrator"],
  [/\bHazLenz advisory output\b/g, "HazLenz AI advisory output"],
  [/\bHazLenz multi-hazard decomposition is advisory only\b/g, "HazLenz AI multi-hazard decomposition is advisory only"],
  [/\bHazLenz causal chain reasoning is advisory only\b/g, "HazLenz AI causal chain reasoning is advisory only"],
  [/\bHazLenz corrective action strategies are advisory only\b/g, "HazLenz AI corrective action strategies are advisory only"],
  [/\bHazLenz risk verification and residual risk reassessment is advisory only\b/g, "HazLenz AI risk verification and residual risk reassessment is advisory only"],
  [/\bHazLenz jurisdiction assessment is advisory and requires qualified legal\/safety review\./g, "HazLenz AI jurisdiction assessment is advisory and requires qualified legal/safety review."],
  [/\bHazLenz semantic expansion is advisory only and utilizes NLP stemming and deterministic mapping\./g, "HazLenz AI semantic expansion is advisory only and utilizes NLP stemming and deterministic mapping."],
  [/\bHazLenz visual evidence reasoning is based on attachment metadata and notes only\. It is not an automated image recognition result\./g, "HazLenz AI visual evidence reasoning is based on attachment metadata and notes only. It is not an automated image recognition result."],
  [/\bHazLenz NLP Dynamic Evidence Generator\b/g, "HazLenz AI Dynamic Evidence Generator"],
  [/\bHazLenz reconstructed\b/g, "HazLenz AI reconstructed"],
  [/\bHazLenz confidence\b/g, "HazLenz AI confidence"],
  [/\bHazLenz mappings\b/g, "HazLenz AI mappings"],
  [/\bHazLenz recommendation\b/g, "HazLenz AI recommendation"],
  [/\bHazLenz engine\b/g, "HazLenz AI engine"],
  [/\bHazLenz\b/g, "HazLenz AI"],

  // The retired product names, taken from the list that
  // src/hazlenz/tests/smoke/hazlenz-field-output-smoke.js already asserts must
  // never appear in HazLenz field output. Only "HazLenz" had a rule, so a residual
  ...
  // `sentinel_*` storage keys or the `hazlenz-v2` API namespace.
