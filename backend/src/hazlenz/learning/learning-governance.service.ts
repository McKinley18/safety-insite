export type HazLenzLearningSignal = {
  source:
    | "workspace_feedback"
...
export class HazLenzLearningGovernanceService {
  evaluate(input: {
    ...
    const approvedSignals: HazLenzLearningSignal[] = [];
    const quarantinedSignals: HazLenzLearningSignal[] = [];
    ...
    for (const signal of feedbackSignals) {
      ...
      const learningSignal: HazLenzLearningSignal = {
        ...
        reason: trusted
          ...
          : "Feedback is not trusted enough to influence HazLenz decisions.",
      };
    ...
    if (quarantinedSignals.length) {
      learningWarnings.push(
        "Some learning signals were quarantined because they were not validated enough to influence HazLenz.",
      );
    }
    ...
    return {
      ...
      finalGovernanceRule:
        "HazLenz may learn from validated feedback and field outcomes, but learning cannot override regulations, source requirements, high-risk review triggers, or qualified human judgment.",
    };
  }
}
