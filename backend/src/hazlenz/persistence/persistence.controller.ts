...
import { HazLenzPersistenceService } from './persistence.service';
import { AuditRecordFilter, AuditRecordType } from './persistence.types';
import { JwtGuard } from '../../auth/guards/jwt.guard';
import { RolesGuard } from '../../auth/guards/roles.guard';
import { Roles } from '../../auth/decorators/roles.decorator';
import { Request } from 'express';
import { UserGovernanceContext, HazLenzRole } from '../workspace-governance-access/workspace-governance.types';

@Controller('hazlenz/persistence')
@UseGuards(JwtGuard, RolesGuard)
export class HazLenzPersistenceController {
  constructor(private readonly service: HazLenzPersistenceService) {}

  private getGovernanceContext(req: Request & { user?: any }): UserGovernanceContext {
      const user = req.user;
      const roleMap: Record<string, HazLenzRole> = {
          'ORG_OWNER': 'owner',
          'SUPER_ADMIN': 'admin',
