...
import { HazLenzAuditRecordEntity } from './audit-record.entity';
...
@Injectable()
export class HazLenzPersistenceService {
  ...
  constructor(
    @Optional()
    @InjectRepository(HazLenzAuditRecordEntity)
    private readonly repository?: Repository<HazLenzAuditRecordEntity>,
  ) {
    ...
    if (this.persistenceMode === 'file') {
      this.ensureDirectory();
        ...
        console.error('FATAL: HazLenz persistence configured for database but repository is unavailable.');
    }
  }
  ...
  private loadFromFile(): HazLenzAuditRecord[] {
    try {
  ...
  private saveToFile(records: HazLenzAuditRecord[]) {
    try {
    ...
    if (!this.auditTableReady) {
      this.auditTableReady = (async () => {
        try {
          ...
          console.warn('[HazLenz persistence] pgcrypto extension could not be ensured; continuing with database UUID support.', error);
        }
  ...
  async save(record: Omit<HazLenzAuditRecord, 'id' | 'createdAt' | 'updatedAt'>): Promise<HazLenzAuditRecord> {
    if (this.repository && this.persistenceMode === 'database') {
    ...
    const newRecord: HazLenzAuditRecord = {
      ...record,
  ...
  async find(filter: AuditRecordFilter, user?: UserGovernanceContext): Promise<HazLenzAuditRecord[]> {
    ...
    let results: HazLenzAuditRecord[] = [];
    ...
    if (this.repository && this.persistenceMode === 'database') {
      try {
        ...
        if (this.isMissingAuditTableError(error)) {
          console.warn('[HazLenz persistence] audit table missing or unavailable; returning empty optional audit records.', error);
          results = [];
        } else {
          throw error;
        }
  ...
  async getById(id: string, user?: UserGovernanceContext): Promise<HazLenzAuditRecord | undefined> {
    let record: HazLenzAuditRecord | undefined;
  ...
  async updateStatus(id: string, status: string, metadataUpdate: Record<string, any> = {}, user?: UserGovernanceContext): Promise<HazLenzAuditRecord | undefined> {
    if (this.repository && this.persistenceMode === 'database') {
      await this.ensureDatabaseAuditTable();
      const entity = await this.repository.findOne({ where: { id } as any });
      if (entity) {
  ...
  private mapEntityToType(entity: HazLenzAuditRecordEntity): HazLenzAuditRecord {
    return {
      id: entity.id,
      type: entity.type,
