...
import { HazLenzAuditRecord } from './persistence.types';
...
export class CryptographicAuditService {
  ...
  constructor() {
    this.signingKey = process.env.HAZLENZ_AUDIT_SIGNING_KEY || 'sentinel-safety-hazlenz-audit-secret-fallback-key-2026';
  }
   ...
   */
  private generateCanonicalPayload(record: Partial<HazLenzAuditRecord>): string {
  ...
  /**
   * Signs a HazLenzAuditRecord by appending an HMAC-SHA256 signature to its metadata
   */
  public signRecord(record: HazLenzAuditRecord): HazLenzAuditRecord {
  ...
  /**
   * Verifies the cryptographic signature of a HazLenzAuditRecord.
   * Employs constant-time timingSafeEqual comparison to protect against timing attacks.
   */
  public verifyRecord(record: HazLenzAuditRecord): boolean {
