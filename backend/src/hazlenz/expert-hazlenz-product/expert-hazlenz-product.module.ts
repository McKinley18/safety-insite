...
import { HazLenzModule } from '../hazlenz.module';
 ...
 *
 * IT IMPORTS `HazLenzModule` because §260 froze the Expert input as being built from a
 * deterministic analysis the SERVER runs, not from one a client posts. No cycle exists:
 * `HazLenzModule` imports `InspectionModule` and neither imports this one.
 */
@Module({
  imports: [
