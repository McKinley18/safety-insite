...
import {
  resolveHazLenzGovernanceContext,
 ...
 *
 * §260 section 1 established the standing trust boundary: the frontend calls `/hazlenz/classify`,
 ...
 *
 * So §260 section 2 froze the flow as `run the deterministic analysis — existing HazLenzV2 path`,
 * and that is what happens below: the SAME `HazLenzService.classify` the customer route calls,
...
export class ExpertAnalysisContextService {
  ...
  constructor(
    private readonly hazlenz: HazLenzService,
  ...
  async build(
    ...
    const governance = resolveHazLenzGovernanceContext(rawUser);
    const classified: any = await this.hazlenz.classify(
      observationText,
