...
import { HazLenzBrainQueryOrchestratorService } from '../query-orchestrator/brain-query-orchestrator.service';
import { HAZLENZ_MECHANISM_BRAIN_REGISTRY } from '../mechanism-brain/mechanism-knowledge.registry';
import {
  HazLenzBrainSnapshot,
  HazLenzBrainSnapshotInput,
...
function normalizeMechanismId(value: unknown, contextText?: string): string | undefined {
  ...
  const direct = HAZLENZ_MECHANISM_BRAIN_REGISTRY.find(
    (record) => normalized(record.mechanismId) === raw,
  );
  if (direct) return direct.mechanismId;

  const label = HAZLENZ_MECHANISM_BRAIN_REGISTRY.find(
  ...
  const trigger = HAZLENZ_MECHANISM_BRAIN_REGISTRY.find((record) =>
...
export class HazLenzBrainSnapshotBuilderService {
  constructor(
    private readonly brainQueryOrchestrator = new HazLenzBrainQueryOrchestratorService(),
  ) {}

  build(input: HazLenzBrainSnapshotInput): HazLenzBrainSnapshot {
    ...
    const situationalAwarenessPacket = this.brainQueryOrchestrator.query({
      ...
      approvedOnly: true,
      limit: 5,
      scenarioLabel: 'hazlenz-reasoning-run',
    });
