export type HazLenzDecisionConfidenceLevel =
  | 'high'
  | 'moderate'
  | 'low'
  | 'hold';

export type HazLenzDecisionDisposition =
  | 'proceed_with_advisory_output'
  | 'proceed_with_human_review'
  | 'hold_for_critical_evidence';

export type HazLenzDecisionConfidenceInput = {
...
export type HazLenzDecisionConfidenceResult = {
  engine: 'safescope_decision_confidence_v1';
  mode: 'read_only_defensibility_assessment';
  input: HazLenzDecisionConfidenceInput;
  confidenceLevel: HazLenzDecisionConfidenceLevel;
  defensibilityScore: number;
  recommendedDisposition: HazLenzDecisionDisposition;
  reasonCodes: string[];
  warnings: string[];
  boundary: {
    readOnly: true;
