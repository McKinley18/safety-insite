import { HazLenzBrainMatch, HazLenzBrainQuery } from '../hazlenz-brain.types';
import { HAZLENZ_REGULATORY_BRAIN_REGISTRY } from './regulatory-knowledge.registry';
import { HazLenzRegulatoryBrainResult } from './regulatory-brain.types';
...
export class HazLenzRegulatoryBrainService {
  query(query: HazLenzBrainQuery): HazLenzRegulatoryBrainResult {
    const text = normalized(query.text);
    const limit = query.limit && query.limit > 0 ? query.limit : 10;

    const matches = HAZLENZ_REGULATORY_BRAIN_REGISTRY
      .map((record): HazLenzBrainMatch => {
        let score = 0;
    ...
    return {
      ...
      query,
      matches,
      totalAvailable: HAZLENZ_REGULATORY_BRAIN_REGISTRY.length,
      boundary: {
        readOnly: true,
