import { HazLenzBrainQuery } from '../hazlenz-brain.types';
import { HazLenzEvidenceBrainRecord } from './evidence-brain.types';
import { HAZLENZ_EVIDENCE_BRAIN_REGISTRY } from './evidence-knowledge.registry';

export type HazLenzEvidenceBrainMatch = {
  record: HazLenzEvidenceBrainRecord;
  score: number;
  matchedFields: string[];
  reasonCodes: string[];
};

export type HazLenzEvidenceBrainQueryResult = {
  engine: 'safescope_evidence_brain';
  mode: 'read_only_defensibility_requirements';
  query: HazLenzBrainQuery;
  matches: HazLenzEvidenceBrainMatch[];
...
export class HazLenzEvidenceBrainService {
  query(query: HazLenzBrainQuery): HazLenzEvidenceBrainQueryResult {
    ...
    const matches = HAZLENZ_EVIDENCE_BRAIN_REGISTRY
      .map((record): HazLenzEvidenceBrainMatch => {
    ...
    return {
      ...
      query,
      matches,
      totalAvailable: HAZLENZ_EVIDENCE_BRAIN_REGISTRY.length,
      boundary: {
        readOnly: true,
        canCreateCitation: false,
        canDeclareViolation: false,
