import {
  HazLenzEvidenceGapIntelligenceInput,
  HazLenzEvidenceGapIntelligenceMatch,
  HazLenzEvidenceGapIntelligenceResult,
  HazLenzEvidenceGapSeverity,
} from './evidence-gap-intelligence.types';
import { HAZLENZ_EVIDENCE_GAP_INTELLIGENCE_REGISTRY } from './evidence-gap-intelligence.registry';
...
const severityRank: Record<HazLenzEvidenceGapSeverity, number> = {
  low: 1,
  medium: 2,
  high: 3,
  critical: 4,
...
function highestSeverity(values: HazLenzEvidenceGapSeverity[]): HazLenzEvidenceGapSeverity | undefined {
  return values.sort((a, b) => severityRank[b] - severityRank[a])[0];
}

export class HazLenzEvidenceGapIntelligenceService {
  query(input: HazLenzEvidenceGapIntelligenceInput): HazLenzEvidenceGapIntelligenceResult {
    ...
    const matches: HazLenzEvidenceGapIntelligenceMatch[] =
      HAZLENZ_EVIDENCE_GAP_INTELLIGENCE_REGISTRY
        .map((record): HazLenzEvidenceGapIntelligenceMatch => {
          let score = 0;
  ...
  list() {
    return [...HAZLENZ_EVIDENCE_GAP_INTELLIGENCE_REGISTRY];
  }
}
