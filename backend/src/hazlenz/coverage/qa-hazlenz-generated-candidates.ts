...
function buildDataSource() {
  const databaseUrl =
    process.env.DATABASE_URL || "postgresql://user:password@db:5432/hazlenz";
...
`;
  ...
  if (criticals.length > 0) {
    criticals.forEach(c => {
      markdown += `* [ ] \`${c.shardKey}\` (File: [${c.filename}](./generated-candidates/${c.filename})) - *Backlog: ${c.backlogTitle}*\n`;
    });
  } else {
...
`;
  ...
  if (rest.length > 0) {
    rest.forEach(c => {
      markdown += `* [ ] \`${c.shardKey}\` (File: [${c.filename}](./generated-candidates/${c.filename})) - *Priority: ${c.priority}*\n`;
    });
  } else {
