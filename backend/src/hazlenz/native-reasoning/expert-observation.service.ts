...
@Injectable()
export class HazLenzExpertObservationService {
  public generateExpertObservations(
    ...
    if (!input.evidenceContract || Object.keys(input.evidenceContract).length === 0) {
      ...
      confidenceCautions.push(
        'HazLenz confidence should be limited until structured evidence is available.',
      );
    ...
    return {
      ...
      sourceBoundary:
        'HazLenz expert observations are deterministic native decision support. They may improve questions, observations, and corrective-action quality, but they cannot invent citations, override standards, or replace qualified review.',
