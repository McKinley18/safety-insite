...
import {
  HazLenzEquipmentComponentMechanism,
  HazLenzEquipmentFailureMode,
  HazLenzEquipmentTaskMechanismRecord,
  HazLenzHarmMechanism,
  HazLenzTaskContext,
} from './equipment-task-mechanism.types';
import { HAZLENZ_EQUIPMENT_TASK_MECHANISM_REGISTRY } from './equipment-task-mechanism.registry';

export type HazLenzEquipmentTaskMechanismDetectionInput = {
  description: string;
  equipmentId?: string;
  taskContext?: HazLenzTaskContext;
};

export type HazLenzEquipmentTaskMechanismDetectionMatch = {
  equipmentId: string;
  ...
  guardrails: HazLenzEquipmentTaskMechanismRecord['guardrails'];
};

export type HazLenzEquipmentTaskMechanismDetectionResult = {
  matched: boolean;
  primaryMatch?: HazLenzEquipmentTaskMechanismDetectionMatch;
  matches: HazLenzEquipmentTaskMechanismDetectionMatch[];
  evidenceGaps: string[];
  cautions: string[];
...
type ScoredCandidate = {
  record: HazLenzEquipmentTaskMechanismRecord;
  component: HazLenzEquipmentComponentMechanism;
  failureMode: HazLenzEquipmentFailureMode;
  score: number;
...
export class HazLenzEquipmentTaskMechanismDetectorService {
  detect(
    input: HazLenzEquipmentTaskMechanismDetectionInput,
  ): HazLenzEquipmentTaskMechanismDetectionResult {
    ...
    if (!normalizedDescription) {
      return {
        ...
        cautions: [
          'HazLenz cannot infer equipment task mechanisms without observation details, equipment context, and task context.',
        ],
    ...
    for (const record of HAZLENZ_EQUIPMENT_TASK_MECHANISM_REGISTRY) {
  ...
  private scoreTaskContext(
    taskContext: HazLenzTaskContext | undefined,
    failureMode: HazLenzEquipmentFailureMode,
    candidate: ScoredCandidate,
  ) {
    if (!taskContext) {
  ...
  private toMatch(candidate: ScoredCandidate): HazLenzEquipmentTaskMechanismDetectionMatch {
    return {
...
function buildFailureModeTerms(failureMode: HazLenzEquipmentFailureMode): string[] {
...
function containsTaskSignal(normalizedDescription: string, task: HazLenzTaskContext): boolean {
