...
import {
  HazLenzEquipmentArchetypeId,
  HazLenzEquipmentArchetypeRecord,
} from './equipment-archetype.types';
import { HAZLENZ_EQUIPMENT_ARCHETYPE_REGISTRY } from './equipment-archetype.registry';
import {
...
export type HazLenzEquipmentArchetypeDetectionInput = {
  description: string;
  taskContext?: HazLenzTaskContext;
};

export type HazLenzEquipmentArchetypeDetectionMatch = {
  ...
  guardrails: HazLenzEquipmentArchetypeRecord['guardrails'];
};

export type HazLenzEquipmentArchetypeDetectionResult = {
  matched: boolean;
  reasoningMode: 'archetype_fallback_context' | 'no_archetype_context';
  primaryMatch?: HazLenzEquipmentArchetypeDetectionMatch;
  matches: HazLenzEquipmentArchetypeDetectionMatch[];
  evidenceGaps: string[];
  cautions: string[];
...
type ScoredArchetypeCandidate = {
  record: HazLenzEquipmentArchetypeRecord;
  score: number;
...
export class HazLenzEquipmentArchetypeDetectorService {
  detect(input: HazLenzEquipmentArchetypeDetectionInput): HazLenzEquipmentArchetypeDetectionResult {
    ...
    if (!description) {
      return {
        ...
        cautions: ['HazLenz cannot infer an equipment archetype without observation details.'],
        detectorNotes: ['Equipment archetype detector skipped because description was empty.'],
      };
    }

    const matches = HAZLENZ_EQUIPMENT_ARCHETYPE_REGISTRY.records
  ...
  private toMatch(candidate: ScoredArchetypeCandidate): HazLenzEquipmentArchetypeDetectionMatch {
    return {
