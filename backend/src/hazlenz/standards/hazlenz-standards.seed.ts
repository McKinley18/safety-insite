...
import { HAZLENZ_CURATED_STANDARDS } from './hazlenz-standards.data';
import {
...
const standards = HAZLENZ_CURATED_STANDARDS;
...
async function run() {
  ...
  // `seed:hazlenz-standards` -- and that was measurably not enough. Against a copy of the
  ...
  console.log(`Seeded ${standards.length} HazLenz standards.`);
  await ds.destroy();
}
