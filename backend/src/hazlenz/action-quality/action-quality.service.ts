import {
  HazLenzActionQualityInput,
  HazLenzActionQualityOutput,
  HazLenzActionQualityRating,
  HazLenzControlLevel,
...
// HazLenzNativeReasoningService, which is itself instantiated but never called from the
// hash-protected hazlenz.service.ts. See native-reasoning.service.ts for the full
// explanation. Do not treat this as a live corrective-action QA check.
export class HazLenzActionQualityService {
  evaluate(input: HazLenzActionQualityInput): HazLenzActionQualityOutput {
    ...
    return {
      ...
      sourceBoundary:
        "HazLenz action quality intelligence evaluates corrective-action completeness, control strength, verification needs, and closure readiness. It cannot invent completed work, bypass verification, reduce required human review, or finalize compliance decisions.",
    };
  }

  private getStrongestControlLevel(text: string): HazLenzControlLevel {
