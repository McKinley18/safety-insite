...
import {
  ...
  KnowledgeQueueActionResult,
  KnowledgeQueueGuardrails,
} from './knowledge-review-queue.service';
...
export class KnowledgeReviewQueueController {
  constructor(
    private readonly store = new KnowledgeReviewQueueStore(),
  ...
  createDraft(input: any, actor: string) {
    ...
    const persisted = this.store.saveRecord(record as KnowledgeRecord);
  ...
  approve(recordId: string, actor: string) {
    ...
    if ((result.result as any).approved === true && isApprovedStatus((result.result as any).record)) {
      const persisted = this.store.saveRecord((result.result as any).record as KnowledgeRecord);
      return this.wrap({
        ...result,
        result: {
  ...
  reject(recordId: string, actor: string, reason: string) {
    ...
    const persisted = this.store.saveRecord(rejectedRecord as KnowledgeRecord);
  ...
  requestMoreInfo(recordId: string, actor: string, reason: string) {
    ...
    const persisted = this.store.saveRecord(updatedRecord as KnowledgeRecord);
  ...
  supersede(recordId: string, replacementInput: any, actor: string) {
    ...
    const persistedOld = this.store.saveRecord(oldSuperseded as KnowledgeRecord);
    const persistedReplacement = this.store.saveRecord(replacementPending as KnowledgeRecord);
...
export type KnowledgeReviewQueueControllerResult<T = unknown> = {
  data: T;
