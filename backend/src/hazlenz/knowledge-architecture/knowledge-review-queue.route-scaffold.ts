import {
  ...
  KnowledgeQueuePersistenceReadiness,
  KnowledgeQueueResponseEnvelope,
} from './knowledge-review-queue.api-types';
import { KnowledgeReviewQueueAudit } from './knowledge-review-queue.audit';
import { KnowledgeReviewQueueController } from './knowledge-review-queue.controller';
import { KnowledgeReviewQueueGuard } from './knowledge-review-queue.guard';
...
export class KnowledgeReviewQueueRouteScaffold {
  private readonly store: KnowledgeReviewQueueStore;
  private readonly controller: KnowledgeReviewQueueController;
  private readonly guard: KnowledgeReviewQueueGuard;
  private readonly audit: KnowledgeReviewQueueAudit;

  constructor(
    store = new KnowledgeReviewQueueStore(),
    service = new KnowledgeReviewQueueService(),
    guard = new KnowledgeReviewQueueGuard(),
    audit = new KnowledgeReviewQueueAudit(),
  ) {
    this.store = store;
    this.controller = new KnowledgeReviewQueueController(store, service);
    this.guard = guard;
    this.audit = audit;
  }
  ...
  ): KnowledgeQueueResponseEnvelope<T> {
  ...
  approve(recordId: string, request: any, actor: KnowledgeQueueActor) {
  ...
  reject(recordId: string, request: any, actor: KnowledgeQueueActor) {
    const before = this.controller.getQueueItem(recordId).data?.result?.record;
  ...
  requestMoreInfo(recordId: string, request: any, actor: KnowledgeQueueActor) {
    const before = this.controller.getQueueItem(recordId).data?.result?.record;
  ...
  supersede(recordId: string, request: any, actor: KnowledgeQueueActor) {
    const before = this.controller.getQueueItem(recordId).data?.result?.record;
  ...
  listActiveRetrievalRecords(actor: KnowledgeQueueActor) {
  ...
  exportQueueSnapshot(actor: KnowledgeQueueActor) {
  ...
  persistenceReadiness(actor: KnowledgeQueueActor): KnowledgeQueueResponseEnvelope<KnowledgeQueuePersistenceReadiness> {
    return this.envelope(
      {
