...
import { KnowledgeQueueActor } from './knowledge-review-queue.api-types';
import { KnowledgeReviewQueueProvider } from './knowledge-review-queue.provider';
import { JwtGuard } from '../../auth/guards/jwt.guard';

@UseGuards(JwtGuard)
@Controller('hazlenz-knowledge/review-queue')
export class KnowledgeReviewQueueHttpController {
  constructor(private readonly provider: KnowledgeReviewQueueProvider) {}

  private getQueueActor(req: any): KnowledgeQueueActor {
    const user = req.user;
    const roleMap: Record<string, KnowledgeQueueActor["role"]> = {
      'ORG_OWNER': 'owner',
      'SUPER_ADMIN': 'admin',
    ...
    const planCodeMap: Record<string, KnowledgeQueueActor["planTier"]> = {
      'company': 'company',
      'plus': 'team',
      'pro': 'team',
      'basic': 'individual',
