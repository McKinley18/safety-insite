...
export interface KnowledgeQueueGuardrails {
  advisoryOnly: true;
...
export interface KnowledgeQueueActionResult<T = unknown> {
  result: T;
  approvalReadiness?: {
    ready: boolean;
    blockers: string[];
...
const QUEUE_GUARDRAILS: KnowledgeQueueGuardrails = {
  advisoryOnly: true,
  doesNotDeclareViolation: true,
  doesNotCreateCitation: true,
  requiresQualifiedReview: true,
...
function needsCitationSupport(record: any): boolean {
  ...
  return (
    ...
    record?.authorityTier === KnowledgeAuthorityTier.CORE ||
    record?.authorityTier === KnowledgeAuthorityTier.ENHANCED
  );
}
...
export class KnowledgeReviewQueueService {
  private ingestion = new KnowledgeIngestionService();
  private approval = new KnowledgeApprovalService();

  private guardrails(): KnowledgeQueueGuardrails {
  ...
  private wrap<T>(
    result: T,
    activeRetrievalEligible: boolean,
    extra: Partial<Omit<KnowledgeQueueActionResult<T>, 'result' | 'guardrails' | 'activeRetrievalEligible'>> = {},
  ): KnowledgeQueueActionResult<T> {
    return {
      result,
  ...
  listQueue(records: KnowledgeRecord[]) {
    ...
    return this.wrap(
      {
        queueItems,
        lifecycleCounts: {
          draft: safeRecords.filter((record: any) => record.status === KnowledgeRecordStatus.DRAFT || statusOf(record) === 'draft').length,
          needs_review: safeRecords.filter((record: any) => record.status === KnowledgeRecordStatus.PENDING_VALIDATION || statusOf(record) === 'needs_review' || statusOf(record) === 'pending_validation').length,
          ...
          retired: safeRecords.filter((record: any) => record.status === KnowledgeRecordStatus.RETIRED || statusOf(record) === 'retired').length,
        },
  ...
  getQueueItem(recordId: string, records: KnowledgeRecord[]) {
  ...
  createDraft(input: any) {
    ...
    const normalized = {
      ...
      status:
        record?.status === KnowledgeRecordStatus.GOVERNED
          ? KnowledgeRecordStatus.PENDING_VALIDATION
          : record?.status ?? KnowledgeRecordStatus.DRAFT,
    };
  ...
  approve(record: KnowledgeRecord, reviewer: string, records: KnowledgeRecord[] = []) {
    const readiness = this.readiness(record, records as any[]);
    if (!readiness.ready) {
      return this.wrap(
        {
    ...
    const normalized = {
      ...approvedRecord,
      status: approvedRecord?.status ?? KnowledgeRecordStatus.GOVERNED,
    };
  ...
  reject(record: KnowledgeRecord, reviewer: string, reason: string) {
  ...
  requestMoreInfo(record: KnowledgeRecord, reviewer: string, reason: string) {
    const updatedRecord = {
      ...(record as any),
  ...
  supersede(oldRecord: KnowledgeRecord, replacementRecord: KnowledgeRecord, reviewer: string) {
    ...
    const replacementPending = {
      ...
      status:
        (replacementRecord as any).status === KnowledgeRecordStatus.GOVERNED
          ? KnowledgeRecordStatus.PENDING_VALIDATION
          : (replacementRecord as any).status ?? KnowledgeRecordStatus.PENDING_VALIDATION,
  ...
  listActiveRetrievalRecords(records: KnowledgeRecord[]) {
