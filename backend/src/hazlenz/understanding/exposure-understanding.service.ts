import {
  HazLenzExposureFrequency,
  HazLenzExposureProximity,
  HazLenzUnderstandingExposure
} from './hazlenz-understanding.types';

export class ExposureUnderstandingService {
  evaluate(normalizedText: string): HazLenzUnderstandingExposure {
    const reasons: string[] = [];

    let workerExposed: boolean | 'unclear' = 'unclear';
    let proximity: HazLenzExposureProximity = 'unknown';
    let exposurePathway = 'unknown';
    let frequency: HazLenzExposureFrequency = 'unknown';
