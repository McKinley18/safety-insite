import { HazLenzTaskType, HazLenzUnderstandingTask } from './hazlenz-understanding.types';

export class TaskUnderstandingService {
  evaluate(normalizedText: string): HazLenzUnderstandingTask {
    const reasons: string[] = [];
    let taskType: HazLenzTaskType = 'unknown';
    let activity = 'unknown';
    let workerRole = 'unknown';

    const taskMap: Array<{ type: HazLenzTaskType; activity: string; terms: string[] }> = [
