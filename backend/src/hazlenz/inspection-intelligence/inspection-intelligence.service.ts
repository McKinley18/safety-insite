...
const RULES: InspectionIntelligenceRule[] = [
  ...
  },
  {
    ...
    initiating: 'A damaged flexible cord is used or located where moisture may be present.', failure: 'Damaged insulation and wet conditions can expose a conductor or create current leakage.', exposure: 'A person may contact exposed energized parts, the cord, or a wet surface; routing may also create a trip path.', consequences: 'Shock, electrocution, burns, secondary fall, or a trip injury if the cord crosses travel.', related: [{ domain: 'walking_working_surfaces', rationale: 'Cord routing can create a trip hazard when it crosses a walkway.', possible: true }],
...
const DOMAIN_SEVERITY: Partial<Record<HazLenzReasoningDomain, number>> = {
  electrical: 100,
...
function ruleScore(
  ...
  classifiedDomain: HazLenzReasoningDomain,
): number {
  ...
  analyze(input: {
    ...
    if (applicabilityCandidateRules.length) {
      applicabilityCandidateRules.forEach((rule, index) => {
        ...
        if (conditionAssessment.controlledDomains.includes(candidateDomain as HazLenzReasoningDomain)) return;
    ...
    const jurisdictions: Array<Exclude<HazLenzJurisdiction, 'unclear'>> = input.jurisdiction === 'unclear'
      ? ['osha_general_industry', 'osha_construction', 'msha']
      : [input.jurisdiction];
    applicabilityCandidateRules.forEach((rule) => {
      if (!rule) return;
      if (conditionAssessment.controlledDomains.includes(rule.hazardFamily as HazLenzReasoningDomain)) return;
