...
import type { HazLenzIntelligenceOrchestrator } from "./orchestration/intelligence-orchestrator.service";
...
import { HazLenzNativeReasoningService } from "./native-reasoning/native-reasoning.service";
import { HazLenzReasoningOrchestratorService } from "./reasoning-orchestrator/reasoning-orchestrator.service";
import { InspectionCitationRecoveryService } from "./inspection-intelligence/inspection-citation-recovery.service";
import { HazLenzReasoningRequest } from "./reasoning-orchestrator/reasoning-orchestrator.types";
...
@Injectable()
export class HazLenzService {
  private classifier = new WeightedClassifierService();
  private bridge = new StandardsBridgeService();
  private nativeReasoningService = new HazLenzNativeReasoningService();
  private reasoningOrchestratorService = new HazLenzReasoningOrchestratorService();
  ...
  constructor(
    ...
    @Optional()
    private readonly persistence?: HazLenzPersistenceService,
  ) {}
  ...
  async classify(
    ...
    if (user) {
      ...
      if (isHazLenzKnowledgeTelemetryEnabled()) {
        logKnowledgeTelemetry("HazLenzService.classify_knowledge_route", {
      ...
      if (
        ...
        try {
          ...
          if (!orchestrator) {
            const { HazLenzIntelligenceOrchestrator } = await import("./orchestration/intelligence-orchestrator.service");
            this.lazyIntelligenceOrchestrator = new HazLenzIntelligenceOrchestrator(
              this.persistence,
              undefined,
              this.access,
            );
      ...
      const standardsTraceability = {
        ...
        scopeFilter: "HazLenzService.applyStandardsScopeFit",
        scopeFilterRole:
