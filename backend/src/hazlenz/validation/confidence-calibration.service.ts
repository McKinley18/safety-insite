    ...
    return {
      ...
      reliabilityStatement:
        calibrationBand === "reliable"
          ? "HazLenz confidence appears supported by available evidence, standards reasoning, and consistency checks."
          : calibrationBand === "use_with_review"
            ? "HazLenz output is usable but should be reviewed against evidence gaps, contradictions, or control effectiveness."
            : "HazLenz output has limited reliability and should not be relied on without supervisor validation.",
    };
  }
}
