...
export class WorkspaceGovernanceAccessService {
  ...
  can(user: UserGovernanceContext, permission: HazLenzPermission, resourceContext?: { workspaceId?: string, inspectionId?: string, jurisdiction?: string }): AccessDecision {
    ...
    switch (permission) {
      ...
      case 'run_classification':
        if (user.role === 'viewer') return this.deny(workspaceId, 'Viewers cannot run HazLenz classification.');
