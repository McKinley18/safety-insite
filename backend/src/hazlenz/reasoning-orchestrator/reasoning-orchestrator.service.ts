import { HAZLENZ_TAXONOMY_REGISTRY } from '../taxonomy/hazlenz-taxonomy.registry';
...
import { HazLenzApplicabilityAnalysisService } from './applicability/applicability-analysis.service';
import { HazLenzCorrectiveActionReasoningService } from './corrective-actions/corrective-action-reasoning.service';
import { HazLenzEquipmentTaskMechanismDetectorService } from '../equipment-knowledge/equipment-task-mechanism-detector.service';
import { HazLenzEquipmentArchetypeDetectorService } from '../equipment-knowledge/equipment-archetype-detector.service';
import { HazLenzMechanismPrecedenceResolverService } from '../mechanism-intelligence/mechanism-precedence-resolver.service';
import { HazLenzBrainSnapshotBuilderService } from '../brain/snapshot-builder/brain-snapshot-builder.service';
import { HazLenzTaskContext } from '../equipment-knowledge/equipment-task-mechanism.types';
import { hasAnyNonNegatedTerm } from './negation-context.util';
import { HazLenzSafetyCalculationsService } from '../safety-calculations/safety-calculations.service';
...
import { KnowledgeRetrievalService } from '../knowledge-architecture/knowledge-retrieval.service';
...
export class HazLenzReasoningOrchestratorService {
  constructor(
    private readonly integrationAdapter = new ApprovedKnowledgeIntegrationAdapterService(),
    private readonly applicabilityAnalysisService = new HazLenzApplicabilityAnalysisService(),
    private readonly correctiveActionReasoningService = new HazLenzCorrectiveActionReasoningService(),
    private readonly equipmentTaskMechanismDetectorService = new HazLenzEquipmentTaskMechanismDetectorService(),
    private readonly equipmentArchetypeDetectorService = new HazLenzEquipmentArchetypeDetectorService(),
    private readonly mechanismPrecedenceResolverService = new HazLenzMechanismPrecedenceResolverService(),
    private readonly brainSnapshotBuilderService = new HazLenzBrainSnapshotBuilderService(),
    private readonly safetyCalculationsService = new HazLenzSafetyCalculationsService(),
  ...
  reason(request: HazLenzReasoningRequest): HazLenzReasoningResult {
  ...
  private buildEquipmentReasoningSummary(
    equipmentTaskMechanismContext: HazLenzReasoningResult['equipmentTaskMechanismContext'],
    equipmentArchetypeContext: HazLenzReasoningResult['equipmentArchetypeContext'],
  ): HazLenzEquipmentReasoningSummary {
  ...
  private normalizeTaskContext(taskContext: string | undefined): HazLenzTaskContext | undefined {
    if (!taskContext) {
      return undefined;
    }
  ...
  private assessJurisdiction(text: string): HazLenzReasoningResult['jurisdictionAssessment'] {
  ...
  private classifyHazard(text: string): HazLenzReasoningResult['hazardClassification'] {
  ...
  private determineDomain(text: string): HazLenzReasoningDomain {
    const normalizedText = normalized(text);
    ...
    for (const entry of Object.values(HAZLENZ_TAXONOMY_REGISTRY)) {
  ...
  private identifyMissingEvidence(
    ...
    const gaps: HazLenzReasoningEvidenceGap[] = [];
  ...
  private calculateConfidence(
    jurisdiction: HazLenzJurisdiction,
    domain: HazLenzReasoningDomain,
    gaps: HazLenzReasoningEvidenceGap[],
    signals: HazLenzApplicabilitySignal[],
    contradictionIntelligence?: any,
  ): { level: HazLenzReasoningConfidence; reasons: string[] } {
    const reasons: string[] = [];
    let score = 0;
  ...
  private resolveCanonicalMechanism(
    ...
    const value = normalized(text);
    const mechanisms: Partial<Record<HazLenzReasoningDomain, string>> = {
