...
export class RegulatoryLiveFetchService {
  ...
  async fetch(options: LiveFetchOptions): Promise<LiveFetchResult> {
    const isLiveAllowed = process.env.HAZLENZ_ALLOW_LIVE_SOURCE_FETCH === 'true';
    ...
    if (!options.allowNetwork || !isLiveAllowed) {
      return {
        ...
        status: 403,
        success: false,
        error: 'Network access denied. Live fetch requires explicit allowNetwork=true and HAZLENZ_ALLOW_LIVE_SOURCE_FETCH=true env var.',
        metadata: {
          url: options.url,
