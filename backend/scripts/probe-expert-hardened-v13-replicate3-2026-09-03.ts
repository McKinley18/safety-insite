/**
 * §154 EXPERT HAZLENZ — HARDENED v9 / v13 REPLICATE 3. THREE-DRAW STABILITY CHARACTERIZATION.
 *
...
/**
 * THE §154 BUDGET, DECLARED HERE AND NOT IN THE FIXTURE MODULE.
 *
...
/**
 * §154 gates. NOT acceptance thresholds -- this is a BASELINE and its job is to measure. They are
...
/** The write-once §154 sidecar, frozen and hashed before this script may spend. */
const SIDECAR_PATH = join(__dirname, '..', '..', 'verification',
  'expert-hazlenz-hardened-v13-replicate3-2026-09-03', 'ADJUDICATION-SIDECAR.json');
const EXPECTED_SIDECAR_SHA =
  '591a027d67232fdd246e87d6e4ab4ad662108ea7f42a2b81665fa5af67a2981f';
...
/** §153's replicate-2 wire, for the three-draw matrix. Read-only. */
const R2_DIR = join(__dirname, '..', '..', 'verification',
  'expert-hazlenz-hardened-v13-replicate2-2026-09-03');
/**
 * A single stored nondeterministic draw, read back from preserved raw wire. Only the fields the
 * three-draw matrix compares are lifted; nothing here is rewritten, rescored, or overwritten.
 */
type Draw = { candidates: Array<{ candidateKey: string; hazardFamily: string;
  assertedConditionState: string }>; clarifications: Array<{ question: string;
  affectedDecision: string; relatesToCandidateKey: string | null }>; summary?: string };
const readDraw = (dir: string): Record<string, Draw> => {
  const out: Record<string, Draw> = {};
  for (const line of readFileSync(join(dir, 'RAW-WIRE.jsonl'), 'utf8').trim().split('\n')) {
    const o = JSON.parse(line) as { rowId: string } & Draw;
    out[o.rowId] = { candidates: o.candidates, clarifications: o.clarifications,
      summary: o.summary };
  }
  return out;
};
 ...
 *
 * §148 kept only key, family and state. §154 needs `evidenceBasis` and `reasoning` because that is
...
(async () => {
  console.log('§154 EXPERT HAZLENZ — HARDENED v9 / v13 REPLICATE 3 — THREE-DRAW STABILITY');
  ...
  // ---- §154 MEASUREMENT LAYER. All three instruments frozen and proved BEFORE any request.
  ...
  check('B.9 the degenerate-output detector is armed and versioned',
    DEGENERATE_DETECTOR_VERSION.endsWith('.v2'), DEGENERATE_DETECTOR_VERSION);
  ...
  check('B.10b replicate 2 artifacts are present and readable — the third draw is worth nothing '
    + 'without the two it is compared against',
    existsSync(join(R2_DIR, 'RAW-WIRE.jsonl')) && existsSync(join(R2_DIR, 'RUN-RECORDS.jsonl')),
    'the §153 evidence directory');
  // The matrix must not discover a parse or coverage problem AFTER money is spent. Prove the two
  // stored draws load, cover the same sixteen rows, and survive detector v2 — now, at $0.00.
  let priorDrawsLoad = 'not attempted';
  try {
    const p1 = readDraw(R1_DIR);
    const p2 = readDraw(R2_DIR);
    const ids = HARDENED_ROWS.map(r => r.source.rowId);
    const covers = (d: Record<string, Draw>) =>
      Object.keys(d).length === ids.length && ids.every(i => d[i] !== undefined);
    const detectorRuns = [p1, p2].every(d => {
      degenerateOutputReport(ids.map(i => ({ rowId: i, candidates: d[i].candidates as never,
        clarifications: d[i].clarifications as never, summary: d[i].summary })));
      return true;
    });
    priorDrawsLoad = covers(p1) && covers(p2) && detectorRuns
      ? `both draws parsed, ${ids.length} rows each, detector v2 ran clean over both`
      : 'PARSED BUT DID NOT COVER THE SIXTEEN ROWS';
  } catch (e) {
    priorDrawsLoad = `THREW: ${(e as Error).message}`;
  }
  check('B.10c both stored draws load, cover all sixteen rows, and run under detector v2 BEFORE '
    + 'spend — the matrix cannot fail after the money is gone',
    priorDrawsLoad.startsWith('both draws parsed'), priorDrawsLoad);
  ...
  // §154 IS NOT A REMEDIATION RUN. The §151 v14 change must still be unimplemented at spend time,
  ...
  // The §154 bridge must be in the text that ACTUALLY GOES TO THE MODEL, placed LAST, and
  ...
  // §154 reads the authorization literally: 10 calls against a 10-request cap leaves NO headroom,
  ...
  const preSpend: PreSpendIdentity = {
    operation: '§154 Expert HazLenz bounded hosted settlement-threshold + affectedDecision routing probe',
  ...
  // ---- THE §154 MEASURE. Reasoned-but-destroyed, reported SEPARATELY and never as recall.
  //
  ...
  // ---- §154 PHASE 5. RAW LINKAGE, computed from the wire BEFORE normalization stripped anything.
  //
  ...
  // ---- §154 THE affectedDecision / HAZARD_SEVERITY PACKET. THE CENTRAL MEASUREMENT.
  //
  ...
  // §149 closed it at 0/10 and §154 must not reopen it.
  ...
  // ---- THE CANDIDATE NON-SUPPRESSION AXIS. §154 must not buy silence with lost hazards, so the
  ...
  const results = {
    operation: '§154 bounded hosted clarification retention-bridge probe',
    ...
    // ---- THE §154 MEASURES. Reported separately, exactly as the authorization requires.
    ...
    // ---- §154 PHASE 5. Four figures, reported separately and reconciled. Never collapsed.
    rawLinkage: {
    ...
    // ---- §154 THE RETENTION-BRIDGE MEASURES. Assembled, never scored.
    // ---- §154 THE CENTRAL MEASUREMENT. Arithmetic computed; correctness adjudicated.
    ...
    unsupportedSettlementRegression: {
      ...
      CANDIDATE_SUPPRESSION_NOTE: '§154 must not buy questions by suppressing hazards, nor silence '
  ...
  // ---- §154 PHASE 8. THE FIXTURE MUST BE BYTE-IDENTICAL TO ITS PRE-SPEND STATE. §151 found four
  ...
  // ---- 3. THE THREE-DRAW STABILITY MATRIX. §152 x §153 x this run, row by row and axis by axis.
  const d1 = readDraw(R1_DIR);
  const d2 = readDraw(R2_DIR);
  const d3: Record<string, Draw> = {};
  for (const r of rows) {
    d3[r.source.rowId] = {
      candidates: (wireCandidates.get(r.source.rowId) ?? []) as never,
      clarifications: (wireClarifications.get(r.source.rowId) ?? []) as never,
  ...
  // POST-HOC COMMON-DETECTOR DIAGNOSTIC. Detector v2 applied to the stored §152 and §153 wire so
  // execution quality is comparable across draws. LABELLED POST HOC. OVERWRITES NOTHING.
  const asCheck = (id: string, d: Draw): DegenerateCheckInput => ({ rowId: id,
    candidates: d.candidates as never, clarifications: d.clarifications as never,
    summary: d.summary });
  const degenR1 = degenerateOutputReport(rows.map(r => asCheck(r.source.rowId, d1[r.source.rowId])));
  const degenR2 = degenerateOutputReport(rows.map(r => asCheck(r.source.rowId, d2[r.source.rowId])));
  const postHocCommonDetector = {
    LABEL: 'POST-HOC COMMON-DETECTOR DIAGNOSTIC',
    detector: DEGENERATE_DETECTOR_VERSION,
    caveat: 'Detector v2 is applied RETROSPECTIVELY to §152 and §153 raw wire, which is preserved. '
      + 'It exists ONLY to make execution quality comparable across draws. IT DOES NOT OVERWRITE '
      + 'THE HISTORICAL §152 OR §153 SCORES, which were computed under the detector in force at the '
      + 'time (§152: none; §153: v1).',
    replicate1_underV2: degenR1.DEGENERATE_ROW_IDS,
    replicate2_underV2: degenR2.DEGENERATE_ROW_IDS,
    replicate3_underV2: degenerate.DEGENERATE_ROW_IDS,
    replicate2_asScoredAtTheTime_underV1: ['HS-K1'],
  };

  const axisEq = (a: string[], b: string[]) =>
    a.slice().sort().join(',') === b.slice().sort().join(',');
  const matrix = rows.map(r => {
    const id = r.source.rowId;
    const draws = [d1[id], d2[id], d3[id]];
    const degenPerDraw = [degenR1.DEGENERATE_ROW_IDS.includes(id),
      degenR2.DEGENERATE_ROW_IDS.includes(id), degenerate.DEGENERATE_ROW_IDS.includes(id)];
    const axis = {
      candidatePresence: draws.map(d => d.candidates.length > 0),
      candidateCount: draws.map(d => d.candidates.length),
      candidateState: draws.map(d => d.candidates.map(c => c.assertedConditionState)),
      candidateFamily: draws.map(d => d.candidates.map(c => c.hazardFamily)),
      clarificationPresence: draws.map(d => d.clarifications.length > 0),
      clarificationCount: draws.map(d => d.clarifications.length),
      affectedDecision: draws.map(d => d.clarifications.map(c => c.affectedDecision)),
      linkage: draws.map(d => d.clarifications.filter(c => c.relatesToCandidateKey).length),
      degenerateExecution: degenPerDraw,
    };
    const stable = {
      candidatePresence: new Set(axis.candidatePresence).size === 1,
      candidateState: axisEq(axis.candidateState[0], axis.candidateState[1])
        && axisEq(axis.candidateState[1], axis.candidateState[2]),
      candidateFamily: axisEq(axis.candidateFamily[0], axis.candidateFamily[1])
        && axisEq(axis.candidateFamily[1], axis.candidateFamily[2]),
      clarificationPresence: new Set(axis.clarificationPresence).size === 1,
      affectedDecision: axisEq(axis.affectedDecision[0], axis.affectedDecision[1])
        && axisEq(axis.affectedDecision[1], axis.affectedDecision[2]),
      linkage: new Set(axis.linkage).size === 1,
    };
    const allStable = Object.values(stable).every(Boolean);
    const clarStates = new Set(draws.map((d, i) => degenPerDraw[i] ? 'DEGENERATE'
      : d.clarifications.length === 0 ? 'SILENT' : `SPOKE:${d.clarifications.length}`));
    const contaminated = degenPerDraw.some(Boolean);
    const classification = contaminated ? 'EXECUTION_CONTAMINATED'
      : allStable ? 'STABLE_3_OF_3'
      : clarStates.size >= 3 ? 'MULTI_STATE_VARIABLE'
      : new Set(axis.clarificationPresence).size > 1 ? 'SINGLE_FLIP'
      : 'MULTI_STATE_VARIABLE';
    return { rowId: id, form: hardenedFixtureByRowId(id)!.form,
      expectation: hardenedFixtureByRowId(id)!.expectation.kind,
      axis, stable, allStable, classification,
      distinctClarificationStates: [...clarStates],
      distinctClarificationStateCount: clarStates.size };
  });

  const cnt = (f: (m: typeof matrix[number]) => boolean) => matrix.filter(f).length;
  const validRows = matrix.filter(m => m.classification !== 'EXECUTION_CONTAMINATED');
  const threeDrawStability = {
    LABEL: 'THREE-DRAW EMPIRICAL STABILITY — NOT a population variance estimate. Three '
      + 'nondeterministic executions exist.',
    ROWS_STABLE_3_OF_3: cnt(m => m.classification === 'STABLE_3_OF_3'),
    ROWS_WITH_SINGLE_FLIP: cnt(m => m.classification === 'SINGLE_FLIP'),
    ROWS_MULTI_STATE_VARIABLE: cnt(m => m.classification === 'MULTI_STATE_VARIABLE'),
    ROWS_EXECUTION_CONTAMINATED: cnt(m => m.classification === 'EXECUTION_CONTAMINATED'),
    perAxis: {
      CLARIFICATION_PRESENCE_3_DRAW_STABILITY:
        `${cnt(m => m.stable.clarificationPresence)}/${matrix.length}`,
      AFFECTED_DECISION_3_DRAW_STABILITY: `${cnt(m => m.stable.affectedDecision)}/${matrix.length}`,
      CANDIDATE_STATE_3_DRAW_STABILITY: `${cnt(m => m.stable.candidateState)}/${matrix.length}`,
      CANDIDATE_FAMILY_3_DRAW_STABILITY: `${cnt(m => m.stable.candidateFamily)}/${matrix.length}`,
      LINKAGE_3_DRAW_STABILITY: `${cnt(m => m.stable.linkage)}/${matrix.length}`,
      CANDIDATE_PRESENCE_3_DRAW_STABILITY:
        `${cnt(m => m.stable.candidatePresence)}/${matrix.length}`,
    },
    behaviouralDispersion: {
      DESCRIPTIVE_ONLY: 'Distinct observed CLARIFICATION states per row across three draws. DO NOT '
        + 'INFER PROBABILITIES FROM THREE DRAWS.',
      validRowsConsidered: validRows.length,
      rowsWith1State: validRows.filter(m => m.distinctClarificationStateCount === 1).length,
      rowsWith2States: validRows.filter(m => m.distinctClarificationStateCount === 2).length,
      rowsWith3States: validRows.filter(m => m.distinctClarificationStateCount === 3).length,
      MEAN_DISTINCT_STATES_PER_VALID_ROW: validRows.length === 0 ? null
        : Number((validRows.reduce((t, m) => t + m.distinctClarificationStateCount, 0)
          / validRows.length).toFixed(3)),
    },
    matrix,
  };
  ...
  console.log(`  3-draw STABLE_3_OF_3     ${threeDrawStability.ROWS_STABLE_3_OF_3}/16`
    + `   single-flip ${threeDrawStability.ROWS_WITH_SINGLE_FLIP}`
    + `   multi-state ${threeDrawStability.ROWS_MULTI_STATE_VARIABLE}`
    + `   contaminated ${threeDrawStability.ROWS_EXECUTION_CONTAMINATED}`);
  console.log(`  per-axis 3-draw          clar ${threeDrawStability.perAxis.CLARIFICATION_PRESENCE_3_DRAW_STABILITY}`
    + `  aff ${threeDrawStability.perAxis.AFFECTED_DECISION_3_DRAW_STABILITY}`
    + `  state ${threeDrawStability.perAxis.CANDIDATE_STATE_3_DRAW_STABILITY}`
    + `  fam ${threeDrawStability.perAxis.CANDIDATE_FAMILY_3_DRAW_STABILITY}`
    + `  link ${threeDrawStability.perAxis.LINKAGE_3_DRAW_STABILITY}`);
  console.log(`  dispersion mean states   ${threeDrawStability.behaviouralDispersion.MEAN_DISTINCT_STATES_PER_VALID_ROW}`
    + `   1-state ${threeDrawStability.behaviouralDispersion.rowsWith1State}`
    + `  2-state ${threeDrawStability.behaviouralDispersion.rowsWith2States}`
    + `  3-state ${threeDrawStability.behaviouralDispersion.rowsWith3States}`);
  console.log(`  post-hoc common detector R1 ${postHocCommonDetector.replicate1_underV2.join(' ')||'none'}`
    + ` | R2 ${postHocCommonDetector.replicate2_underV2.join(' ')||'none'}`
    + ` | R3 ${postHocCommonDetector.replicate3_underV2.join(' ')||'none'}`);
