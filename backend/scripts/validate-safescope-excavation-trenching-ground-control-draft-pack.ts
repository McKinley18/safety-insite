...
const PACK_PATH = path.resolve(__dirname, '../../safescope-data/approved-knowledge/draft-candidates/excavation-trenching-ground-control-draft-candidates.v1.json');
...
async function validate() {
  ...
  if (pack.packId !== "excavation-trenching-ground-control-draft-candidates") {
  ...
  const keywords = ['excavation', 'trench', 'cave', 'protective', 'soil', 'spoil', 'egress', 'water', 'utility', 'competent'];
  ...
  const mechanisms = ['cave_in', 'soil_collapse', 'engulfment', 'fall_into_excavation', 'utility_strike'];
  for (const mech of mechanisms) {
      if (!packString.includes(mech)) {
          console.error(`Required mechanism missing: ${mech}`);
          process.exit(1);
      }
  }
