/**
 * L3-2b -- scores a captured run at THREE TIERS, which Phase 9 requires be kept apart.
 *
...
function scoreTier(records: Rec[], tier: Tier) {
  ...
  for (const r of records) {
    ...
    if (r.expect.hazardEstablished === true) {
      ...
      if (asserts) { g.falseActive += 1; g.falseActiveIds.push(r.id); }
      g.stateScored += 1; if (!asserts) g.stateCorrect += 1;
    }
  }
...
function main(): void {
  ...
  const report = {
    ...
    rows: records.map(r => ({
      id: r.id, failureMode: r.failureMode,
  ...
  process.stdout.write(JSON.stringify({ scenarios: report.scenarios, tiers: report.tiers, pipeline: report.pipeline }, null, 2) + '\n');
}
