/**
 * §210D -- MINIMAL RESIDUAL-REMEDIATION HOSTED CONFIRMATION. BOUNDED PROVIDER EXECUTION.
 * ZERO DATABASE OPERATIONS.
 *
 * Five FIRST_PASS calls, one per frozen §210D case, against
 * `hazlenz.expert.first-pass-instruction.210c-R1-R3`. DEVELOPMENT evidence. NOT acceptance.
 *
 ...
 * ==================== BOUNDS ====================
 *
 *   calls               5, exactly one per frozen case
 *   retries             0
 *   verifier calls      0
 *   governed stage      0
 *   HARD SPEND CEILING  USD 0.55, enforced before every call on observed economics
...
// ---- .env, exactly as §199, §206, §208 and §210B-3B load it: never overwriting an exported var.
{
...
import {
  EXPERT_FIRST_PASS_210C_SYSTEM_PROMPT, EXPERT_FIRST_PASS_INSTRUCTION_210C_VERSION,
  instructionIdentities210c,
...
const PREREG_DIR = join(
  ROOT, 'verification', 'expert-hazlenz-210d-confirmation-preregistration-2026-09-09');
const EVID = join(ROOT, 'verification', 'expert-hazlenz-210d-hosted-confirmation-2026-09-09');

const EXECUTOR_VERSION = 'hazlenz.expert.210d.confirmation-execution.v1' as const;
const AUTHORIZATION_REFERENCE = 'SECTION-210D-PRODUCT-OWNER-EXECUTION-AUTHORIZATION-2026-09-09';
const AUTHORIZED_PREREGISTRATION_SHA =
  '55510f9cc4e9424d543332e1c2f5db5a86d26757adba28d21730cc882420f30f' as const;
const AUTHORIZED_PLAIN_INSTRUCTION_IDENTITY =
  'b243c323af82031c6f733af80f456c4bc61d52efafd89b5c25806a5d8a83b0e6' as const;

const HARD_SPEND_CEILING_USD = 0.55;
const FIRST_PASS_MAX_TOKENS = 4000; // frozen. Outputs are NOT truncated to fit a budget.
const CALLS = 5;
const WORST_INPUT_TOKENS = 27_000;
...
/** Gates call 1 only, before any economics have been observed. Frozen projection / 5. */
const PRIOR_PER_CALL_USD = 0.3916 / CALLS;

const LEDGER = join(EVID, 'CALL-LEDGER-210D.jsonl');
const RAW = join(EVID, 'RAW-FIRST-PASS-210D.jsonl');
const PROJECTION = join(EVID, 'PROJECTION-210D.jsonl');

const sha = (s: string): string => createHash('sha256').update(s, 'utf8').digest('hex');

// ================================================================ pre-execution gate

const preregRaw = readFileSync(join(PREREG_DIR, 'CONFIRMATION-PREREGISTRATION-210D.json'));
...
console.log('================ §210D EXECUTION GATE (zero provider calls so far)');
...
if (ids.withoutGovernedBinding.newIdentity
  !== prereg.instructionUnderTest.plainIdentitySha256) {
...
if (frozen.length !== CONFIRMATION_STIMULI.length) blockers.push('stimulus count differs');
for (let i = 0; i < Math.min(frozen.length, CONFIRMATION_STIMULI.length); i += 1) {
  const f = frozen[i]; const l = CONFIRMATION_STIMULI[i] as any;
...
if (CONFIRMATION_STIMULI.length !== CALLS) {
  blockers.push(`${CONFIRMATION_STIMULI.length} cases, expected ${CALLS}`);
}
for (const s of CONFIRMATION_STIMULI) {
  if (s.governedEvidence.length !== 0) {
    blockers.push(`${s.caseId}: §210D is capability-ABSENT on every case`);
  }
}
...
if (blockers.length > 0) {
  console.error('\n§210D ABORT BEFORE PROVIDER EXECUTION. ZERO PROVIDER CALLS WERE MADE. '
...
function build(s: (typeof CONFIRMATION_STIMULI)[number]): Built {
  ...
  return {
    caseId: s.caseId,
    input,
    // Capability-ABSENT on every case: the PLAIN §210C prompt, never the governed variant.
    systemPrompt: EXPERT_FIRST_PASS_210C_SYSTEM_PROMPT,
...
const CASES: readonly Built[] = CONFIRMATION_STIMULI.map(build);
...
async function providerCall(c: Built): Promise<{
  ...
  if (wire.includes('cache_control')) {
    throw new Error('§210D ABORT: caching is disabled and the body carries cache_control.');
  }
  if (wire.includes('governedEvidenceSourceIds')) {
    throw new Error(`§210D ABORT: ${c.caseId} is capability-ABSENT and its request carries a `
      + 'governed-binding property. Refusing to transmit.');
  }
  if (sha(c.systemPrompt) !== AUTHORIZED_PLAIN_INSTRUCTION_IDENTITY) {
    throw new Error('§210D ABORT: the system prompt is not the authorized plain instruction.');
  }
  ...
  const record: CallRecord = {
    ...
    instructionVersion: EXPERT_FIRST_PASS_INSTRUCTION_210C_VERSION,
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < CASES.length; i += 1) {
    ...
    appendFileSync(PROJECTION, `${JSON.stringify({
      ...
      clarifications: Array.isArray((parsed as any).decisionCriticalClarifications)
        // EXECUTOR_DEFECT_1 corrected. The back-reference is read under the CONTRACT'S field name,
        // taken from the contract module rather than retyped, so a rename cannot silently
        // reintroduce the defect. Reading `relatesToDeclarationId` recorded PROPERTY_ABSENT on
        // every case and said nothing about what the model wrote.
    ...
    console.log(`${out.record.failureClass}  decl=${declarations.length}`
      + `(exp ${c.expectedDeclarationCount})  in=${String(out.record.inputTokens)} `
  ...
  const ESTIMATED_DELTA = prereg.tokenAndSpendProjection
    .section210cStaticDeltaTokensEstimated as number;

  const report = {
    artifact: 'SECTION_210D_TOKEN_REPORT',
    executorVersion: EXECUTOR_VERSION,
    authorizationReference: AUTHORIZATION_REFERENCE,
    preregistrationSha256: preregSha,
    instructionVersion: EXPERT_FIRST_PASS_INSTRUCTION_210C_VERSION,
    ...
    comparisonWith210B3B: {
      section210b3bMedianInputTokens: B3B_MEDIAN_INPUT,
      section210b3bMedianOutputTokens: B3B_MEDIAN_OUTPUT,
      section210dMedianInputTokens: medIn,
      section210dMedianOutputTokens: medOut,
      observedMedianInputDelta: medIn === null ? null : medIn - B3B_MEDIAN_INPUT,
      section210cEstimatedStaticDeltaTokens: ESTIMATED_DELTA,
      consistencyStatement: 'The observed median-input delta is NOT a clean measurement of the '
        + '§210C static delta. The §210D observations are different texts from the §210B-3B '
        + 'stimuli, with different lengths and different per-case schema enums, so per-case input '
        + 'varies for several reasons at once. The comparison is reported as broadly consistent or '
        + 'not, without claiming precision an uncontrolled comparison cannot support.',
    },
    productionEconomicsClaim: 'NONE. Five development cases do not establish production token '
      + 'economics and none is claimed.',
    stopped,
    timestamp: new Date().toISOString(),
  };
  writeFileSync(join(EVID, 'TOKEN-REPORT-210D.json'), `${JSON.stringify(report, null, 2)}\n`);
  ...
  console.log(`  cumulative cost        : USD ${spendUsd.toFixed(4)} `
    + `(ceiling ${HARD_SPEND_CEILING_USD.toFixed(2)}, projected 0.3916)`);
  console.log(`  cost per observation   : USD ${(spendUsd / Math.max(all.length, 1)).toFixed(4)}`);
  console.log(`  median input tokens    : ${String(medIn)} (§210B-3B ${B3B_MEDIAN_INPUT})`);
