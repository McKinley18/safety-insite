/**
 * §210F -- FINAL RESIDUAL FIRST-PASS HOSTED CONFIRMATION. BOUNDED PROVIDER EXECUTION.
 * ZERO DATABASE OPERATIONS.
 *
 * Four FIRST_PASS calls, one per frozen §210F case, against
 * `hazlenz.expert.first-pass-instruction.210e-R4-R7`. DEVELOPMENT evidence. NOT acceptance.
 *
 ...
 * ==================== BOUNDS ====================
 *
 *   calls               4, exactly one per frozen case
 *   retries             0
 *   verifier calls      0
 *   governed stage      0
 *   HARD SPEND CEILING  USD 0.45, enforced before every call on observed economics
...
import {
  EXPERT_FIRST_PASS_210E_SYSTEM_PROMPT, EXPERT_FIRST_PASS_INSTRUCTION_210E_VERSION,
  instructionIdentities210e,
...
import { FINAL_CONFIRMATION_STIMULI } from './lib/section-210f-final-confirmation-preregistration';

const ROOT = join(__dirname, '..', '..');
const PREREG_DIR = join(
  ROOT, 'verification', 'expert-hazlenz-210f-final-confirmation-preregistration-2026-09-09');
const EVID = join(ROOT, 'verification', 'expert-hazlenz-210f-hosted-confirmation-2026-09-09');

const EXECUTOR_VERSION = 'hazlenz.expert.210f.final-confirmation-execution.v1' as const;
const AUTHORIZATION_REFERENCE = 'SECTION-210F-PRODUCT-OWNER-EXECUTION-AUTHORIZATION-2026-09-09';
const AUTHORIZED_PREREGISTRATION_SHA =
  '2c186aa15734728c81e88e54ec313cba48377083ecc5e775ed01bf1281d0c24a' as const;
...
const AUTHORIZED_GOVERNED_INSTRUCTION_IDENTITY =
  'e6794aaf894df3dbc186ce38d0c16b2fc69e6a0fa758c1a110ac1e640d7e7c2f' as const;

const HARD_SPEND_CEILING_USD = 0.45;
...
/** Gates call 1 only, before any economics have been observed. Frozen projection / 4. */
const PRIOR_PER_CALL_USD = 0.3224 / CALLS;

const LEDGER = join(EVID, 'CALL-LEDGER-210F.jsonl');
const RAW = join(EVID, 'RAW-FIRST-PASS-210F.jsonl');
const PROJECTION = join(EVID, 'PROJECTION-210F.jsonl');
const CONTRACT = join(EVID, 'CONTRACT-VALIDATION-210F.jsonl');

const sha = (s: string): string => createHash('sha256').update(s, 'utf8').digest('hex');

// ================================================================ pre-execution gate

const preregRaw = readFileSync(join(PREREG_DIR, 'FINAL-CONFIRMATION-PREREGISTRATION-210F.json'));
...
console.log('================ §210F EXECUTION GATE (zero provider calls so far)');
...
if (frozen.length !== FINAL_CONFIRMATION_STIMULI.length) blockers.push('stimulus count differs');
for (let i = 0; i < Math.min(frozen.length, FINAL_CONFIRMATION_STIMULI.length); i += 1) {
  const f = frozen[i]; const l = FINAL_CONFIRMATION_STIMULI[i] as any;
...
if (FINAL_CONFIRMATION_STIMULI.length !== CALLS) {
  blockers.push(`${FINAL_CONFIRMATION_STIMULI.length} cases, expected ${CALLS}`);
}
for (const s of FINAL_CONFIRMATION_STIMULI) {
  if (s.governedEvidence.length !== 0) {
    blockers.push(`${s.caseId}: §210F is capability-ABSENT on every case`);
  }
}
...
if (blockers.length > 0) {
  console.error('\n§210F ABORT BEFORE PROVIDER EXECUTION. ZERO PROVIDER CALLS WERE MADE. '
...
function build(s: (typeof FINAL_CONFIRMATION_STIMULI)[number]): Built {
  ...
  return {
    ...
    systemPrompt: EXPERT_FIRST_PASS_210E_SYSTEM_PROMPT,
...
const CASES: readonly Built[] = FINAL_CONFIRMATION_STIMULI.map(build);
...
async function providerCall(c: Built): Promise<{
  ...
  if (wire.includes('cache_control')) {
    throw new Error('§210F ABORT: caching is disabled and the body carries cache_control.');
  }
  if (wire.includes('governedEvidenceSourceIds')) {
    throw new Error(`§210F ABORT: ${c.caseId} is capability-ABSENT and its request carries a `
      + 'governed-binding property. Refusing to transmit.');
  }
  if (sha(c.systemPrompt) !== AUTHORIZED_PLAIN_INSTRUCTION_IDENTITY) {
    throw new Error('§210F ABORT: the system prompt is not the authorized plain instruction.');
  }
  ...
  const record: CallRecord = {
    ...
    instructionVersion: EXPERT_FIRST_PASS_INSTRUCTION_210E_VERSION,
...
async function main(): Promise<void> {
  ...
  console.log('\n================ FIRST PASS, 4 CALLS, UNCACHED, CAPABILITY-ABSENT');
  ...
  /**
   * §210D's OWN measured figures, read from its persisted token report rather than retyped. A
   * retyped literal is exactly the EXECUTOR_DEFECT_1 shape, and a wrong comparison number is a
   * defect even when it changes no verdict.
   */
  const d210Report = JSON.parse(readFileSync(join(
    ROOT, 'verification', 'expert-hazlenz-210d-hosted-confirmation-2026-09-09',
    'TOKEN-REPORT-210D.json'), 'utf8')) as any;
  const D210_MEDIAN_INPUT = d210Report.medianInputTokens as number;
  const D210_MEDIAN_OUTPUT = d210Report.medianOutputTokens as number;
  const D210_CUMULATIVE_USD = d210Report.cumulativeCostUsd as number;
  const D210_PER_CALL_USD = d210Report.costPerObservationUsd as number;

  const B3B_MEDIAN_INPUT = prereg.tokenAndSpendProjection.section210b3bMedianInputTokens as number;
  const B3B_MEDIAN_OUTPUT = prereg.tokenAndSpendProjection.section210b3bMedianOutputTokens as number;
  const CUMULATIVE_DELTA =
    prereg.tokenAndSpendProjection.cumulativeStaticDeltaTokensEstimated as number;
  const PROJECTED_IN = prereg.tokenAndSpendProjection.projectedInputTokensPerCall as number;

  const report = {
    artifact: 'SECTION_210F_TOKEN_REPORT',
    executorVersion: EXECUTOR_VERSION,
    authorizationReference: AUTHORIZATION_REFERENCE,
    preregistrationSha256: preregSha,
    instructionVersion: EXPERT_FIRST_PASS_INSTRUCTION_210E_VERSION,
    ...
    comparisonWith210D: {
      ...
      section210dMedianInputTokens: D210_MEDIAN_INPUT,
      section210dMedianOutputTokens: D210_MEDIAN_OUTPUT,
      section210dCumulativeCostUsd: D210_CUMULATIVE_USD,
      section210dCostPerObservationUsd: D210_PER_CALL_USD,
      section210fMedianInputTokens: medIn,
      section210fMedianOutputTokens: medOut,
      cumulativeEstimatedStaticDeltaTokens: CUMULATIVE_DELTA,
      consistencyStatement: 'The observed median-input delta is NOT a clean measurement of the '
        + '§210E static delta. The §210F observations are different texts from the §210D stimuli, '
        ...
        + 'precision an uncontrolled comparison cannot support. §210D figures are read from its '
        + 'own token report at adjudication time and restated there.',
    },
    productionEconomicsClaim: 'NONE. Four development cases do not establish production token '
      + 'economics and none is claimed.',
    stopped,
    timestamp: new Date().toISOString(),
  };
  writeFileSync(join(EVID, 'TOKEN-REPORT-210F.json'), `${JSON.stringify(report, null, 2)}\n`);
  ...
  console.log(`  cumulative cost        : USD ${spendUsd.toFixed(4)} `
    + `(ceiling ${HARD_SPEND_CEILING_USD.toFixed(2)}, projected 0.3224)`);
  ...
  console.log(`  median output tokens   : ${String(medOut)} (§210B-3B ${B3B_MEDIAN_OUTPUT})`);
