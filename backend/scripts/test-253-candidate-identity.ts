 ...
 * identity. Writes SECTION-253-CANDIDATE-IDENTITY-V2-3.json.
 */
...
async function main(): Promise<void> {
  ...
  const alongsideControl = readAlongsideControlBinding253(readFileSync(
    join(BACKEND, `${C}expert-247-role-justification-projection.ts`), 'utf8'));
  const io = {
  ...
  const real = deriveCandidateIdentity253(io);
  ...
  ok('P9 element 6 resolves to the §253 schema successor',
    real.elements.find(e => e.n === 6)?.resolvedTo === 'buildExpert253WireSchema');
  ok('P10 element 24 binds the finalized alongsideControlConsidered semantics',
    real.elements.find(e => e.n === 24)?.resolvedTo === FIRST_PASS_CONTRACT_253_VERSION);
  ok('P11 the candidate binds the §253 contract version',
    real.boundContractVersion === FIRST_PASS_CONTRACT_253_VERSION);

  // ---------------- negative fixtures
  console.log('\n---- negative fixtures: the identity must FAIL ----');
  const neg = (id: string, over: Partial<typeof io>, why: string): void => {
    const r = deriveCandidateIdentity253({ ...io, ...over } as any);
  ...
  neg('N5 the §239 schema invoked while the §253 schema exists',
  ...
  neg('N12 the §247 schema invoked while the §253 successor exists',
    { production: { ...production, wireSchema: buildExpert247WireSchema(INPUT, governed),
      body: assembleFrom({ ...valid.captured, wireSchema: buildExpert247WireSchema(INPUT, governed) }).body } },
    'reverting to the schema that admits alongsideControlConsidered: null must change the candidate');
  neg('N13 the adapter path left on the §247 schema while production moved to §253',
    { adapter: { ...adapter, wireSchema: buildExpert247WireSchema(INPUT, governed) } },
    'the two paths must invoke the same schema');
  neg('N14 the cessation fields nullable again',
    { alongsideControl: { ...alongsideControl, cessationFieldsNonNullable: false } },
    'undoing the repair must change the candidate');
  neg('N15 the transmitted description invites null again',
    { alongsideControl: { ...alongsideControl, transmittedDescriptionInvitesNull: true } },
    'reinstating the contradiction must change the candidate');
  neg('N16 the §253 schema no longer reduces to §247',
    { alongsideControl: { ...alongsideControl, reducesToSection247ByteForByte: false } },
    'a successor that cannot reduce to its base is not an additive successor');
  ...
  writeFileSync(join(OUT, 'SECTION-253-CANDIDATE-IDENTITY-V2-3.json'), JSON.stringify({
    section: '253', generated: '2026-09-12', providerCalls: 0, databaseOperations: 0,
    version: 'v2.3',
    supersedes: 'SECTION-252-CANDIDATE-IDENTITY-V2-2.json (v2.2), which bound the §247 schema head. '
      + 'That document is left byte-unchanged and remains a true statement about the pre-§253 head.',
