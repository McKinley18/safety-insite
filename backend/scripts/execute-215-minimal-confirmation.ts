/**
 * §215 PHASE B -- FROZEN MINIMAL VERIFIER CONFIRMATION EXECUTION.
...
import { MAX_VERIFIER_CALLS, SPEND_CEILING_USD } from './lib/expert-215-confirmation-instrument';
import { assemble215 } from './lib/expert-215-assembly';

const ROOT = join(__dirname, '..', '..');
const EVID = join(ROOT, 'verification',
  'expert-hazlenz-215-minimal-verifier-confirmation-2026-09-09');
const PREREG = join(EVID, 'CONFIRMATION-PREREGISTRATION-215.json');
const FROZEN_DIGEST = 'a60779493d3ec5bff548c0aa6fee1888293fd9c2bb99f207aefca40052ad703c';

const EXECUTOR_VERSION = 'hazlenz.expert.215.minimal-confirmation-execution.v1';
...
console.log('§215 PHASE B PREFLIGHT — nothing is transmitted until every check passes\n');
...
pf('frozen §215 digest', sha(preregBytes) === FROZEN_DIGEST, sha(preregBytes));
...
if (checks.some(c => !c.ok)) {
  console.log('\n§215 PREFLIGHT FAILED. PROVIDER CALLS = 0. Nothing was transmitted.');
  process.exit(1);
}
writeFileSync(join(EVID, 'PREFLIGHT-215.json'), JSON.stringify({
...
async function providerCall(b: Built): Promise<Record<string, any>> {
  if (callsMade >= MAX_VERIFIER_CALLS) throw new Error('§215 HARD CALL CEILING reached');
  if (spendUsd + WORST_CALL_USD > SPEND_CEILING_USD) {
    stopped = 'SPEND_CEILING_WOULD_BE_EXCEEDED';
    throw new Error(`§215 SPEND CEILING USD ${SPEND_CEILING_USD} would be exceeded `
  ...
  if (wire.includes('"strict"') || wire.includes('cache_control')) {
    throw new Error('§215 ABORT: forbidden field in the request body. Not transmitting.');
  }
  ...
  const record = {
    callIndex: callsMade,
    recordKind: 'RAW_VERIFIER_CALL_215',
  ...
  write('RAW-VERIFIER-215.jsonl', record);
  write('CALL-LEDGER-215.jsonl', {
...
void (async () => {
  ...
  writeFileSync(join(EVID, 'EXECUTION-SUMMARY-215.json'), JSON.stringify({
    executorVersion: EXECUTOR_VERSION, frozenDigest: FROZEN_DIGEST,
  ...
  const rows = readFileSync(join(EVID, 'RAW-VERIFIER-215.jsonl'), 'utf8').trim().split('\n')
    .map(l => JSON.parse(l));
  for (const r of rows) {
    ...
    write('CONTRACT-VALIDATION-215.jsonl', {
