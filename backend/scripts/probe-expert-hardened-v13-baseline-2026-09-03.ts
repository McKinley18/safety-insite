/**
 * §152 EXPERT HAZLENZ — BOUNDED HOSTED CLARIFICATION RETENTION-BRIDGE PROBE.
 *
 ...
 *
 * ==================== THE CORRECTED TRUE-CONTRADICTION SEMANTICS (§152 PHASE 6) ====================
 *
...
const OUT = join(ROOT, 'verification', 'expert-hazlenz-hardened-v13-baseline-2026-09-03');

/**
 * THE §152 BUDGET, DECLARED HERE AND NOT IN THE FIXTURE MODULE.
 *
...
/**
 * §152 gates. NOT acceptance thresholds -- this is a BASELINE and its job is to measure. They are
...
const SCRIPT = 'backend/scripts/probe-expert-hardened-v13-baseline-2026-09-03.ts';
 ...
 *
 * §148 kept only key, family and state. §152 needs `evidenceBasis` and `reasoning` because that is
...
(async () => {
  console.log('§152 EXPERT HAZLENZ — BOUNDED HOSTED THRESHOLD/ARBITRATION PROBE');
  ...
  // §152 IS NOT A REMEDIATION RUN. The §151 v14 change must still be unimplemented at spend time,
  ...
  // The §152 bridge must be in the text that ACTUALLY GOES TO THE MODEL, placed LAST, and
  ...
  // §152 reads the authorization literally: 10 calls against a 10-request cap leaves NO headroom,
  ...
  const preSpend: PreSpendIdentity = {
    operation: '§152 Expert HazLenz bounded hosted settlement-threshold + affectedDecision routing probe',
  ...
  // ---- THE §152 MEASURE. Reasoned-but-destroyed, reported SEPARATELY and never as recall.
  //
  ...
  // ---- §152 PHASE 5. RAW LINKAGE, computed from the wire BEFORE normalization stripped anything.
  //
  ...
  // ---- §152 THE affectedDecision / HAZARD_SEVERITY PACKET. THE CENTRAL MEASUREMENT.
  //
  ...
  // §149 closed it at 0/10 and §152 must not reopen it.
  ...
  // ---- THE CANDIDATE NON-SUPPRESSION AXIS. §152 must not buy silence with lost hazards, so the
  ...
  const results = {
    operation: '§152 bounded hosted clarification retention-bridge probe',
    ...
    // ---- THE §152 MEASURES. Reported separately, exactly as the authorization requires.
    ...
    // ---- §152 PHASE 5. Four figures, reported separately and reconciled. Never collapsed.
    rawLinkage: {
    ...
    // ---- §152 THE RETENTION-BRIDGE MEASURES. Assembled, never scored.
    // ---- §152 THE CENTRAL MEASUREMENT. Arithmetic computed; correctness adjudicated.
    ...
    unsupportedSettlementRegression: {
      ...
      CANDIDATE_SUPPRESSION_NOTE: '§152 must not buy questions by suppressing hazards, nor silence '
  ...
  // ---- §152 PHASE 8. THE FIXTURE MUST BE BYTE-IDENTICAL TO ITS PRE-SPEND STATE. §151 found four
  ...
  writeFileSync(join(OUT, 'RESULTS-SUMMARY.json'),
    JSON.stringify({ ...results, instrumentIntegrity }, null, 1) + '\n');
