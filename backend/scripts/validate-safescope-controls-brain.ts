...
import { HazLenzControlsBrainService } from '../src/hazlenz/brain/controls-brain/controls-brain.service';
...
const service = new HazLenzControlsBrainService();

assert(HAZLENZ_CONTROLS_BRAIN_REGISTRY.length >= 15, 'Controls Brain should include at least 15 records.');

for (const record of HAZLENZ_CONTROLS_BRAIN_REGISTRY) {
...
const uncoveredTemplates = CORRECTIVE_ACTION_TEMPLATE_REGISTRY
  ...
  .filter((domain) => !HAZLENZ_CONTROLS_BRAIN_REGISTRY.some((record) => record.hazardDomains.includes(domain)));
...
console.log('✅ HazLenz Controls Brain validation passed.');
console.log(`Corrective action template domains covered: ${CORRECTIVE_ACTION_TEMPLATE_REGISTRY.length}`);
console.log(`Controls Brain records: ${HAZLENZ_CONTROLS_BRAIN_REGISTRY.length}`);
