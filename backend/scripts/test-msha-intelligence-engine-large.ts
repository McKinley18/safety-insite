...
async function runAudit() {
  ...
  let total = 0, primaryMatches = 0, totalMatches = 0;
  ...
  for (const cat of categories) {
    for (let i = 0; i < 25; i++) {
        ...
        total++;
        const allMatches = [...res.primary, ...res.secondary, ...res.additional];
        if (allMatches.length > 0) totalMatches++;
        if (res.primary.length > 0 && i < 10) primaryMatches++;
    }
  }
  console.log('--- AUDIT RESULTS v1.3.1 ---');
  console.log('Total:', total);
  console.log('Primary Cluster Match Rate:', ((primaryMatches/total)*100).toFixed(2) + '%');
  console.log('Total Coverage:', ((totalMatches/total)*100).toFixed(2) + '%');
  await ds.destroy();
}
runAudit();
