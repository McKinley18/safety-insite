...
const E194 = V('expert-hazlenz-verifier-regulatory-basis-2026-09-06');
...
const L: string[] = [];
L.push('§194 SOURCE INTEGRITY GATE — REGULATORY-BASIS REPRESENTATION');
...
L.push('');
L.push('FILES ADDED BY §194');
L.push('  NEW  backend/scripts/lib/expert-verifier-instruction-v3-2.ts');
L.push('  NEW  backend/scripts/lib/expert-verifier-contract-v3-2.ts');
L.push('  NEW  backend/scripts/test-expert-194-regulatory-basis.ts');
L.push('  NEW  backend/scripts/verify-194-source-integrity-2026-09-06.ts');
...
L.push('');
writeFileSync(join(E194, 'SOURCE-INTEGRITY.txt'), `${L.join('\n')}\n`);
console.log(L.join('\n'));
