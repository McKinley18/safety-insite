...
async function runAudit() {
  ...
  const tests = JSON.parse(fs.readFileSync(path.join(__dirname, '../src/intelligence-library/data/5k-real-scenarios.json'), 'utf8'));
  
  let matches = 0, aliasMatches = 0, primaryCit = 0, total = tests.length, failures = [];

  for (const t of tests) {
    const mRes = matchService.match(t.rawDescription, t.hazardFamily, t.industryMode);
    const sRes = await stdService.suggest(t.rawDescription, t.hazardFamily, t.industryMode === 'MSHA' ? 'MSHA' : 'OSHA', 5);
    
    if (mRes.primaryConditionId === t.expectedConditionId) matches++;
    else if (t.expectedConditionId !== 'other_uncertain' && mRes.primaryConditionId !== 'other_uncertain') aliasMatches++;
    else if (t.expectedConditionId !== 'other_uncertain') failures.push({ input: t.rawDescription, expected: t.expectedConditionId, actual: mRes.primaryConditionId });
  ...
  const report = {
    ...
    aliasAdjustedConditionAccuracy: (((matches + aliasMatches) / total) * 100).toFixed(2) + '%',
