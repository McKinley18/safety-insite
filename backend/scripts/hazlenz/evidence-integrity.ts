 ...
 *
 * ---------------------------------------------------------------------------------------------
 * `verification/current/` IS EXCLUDED, BECAUSE IT IS THE LIVING INDEX AND NOT EVIDENCE.
 *
 * §263 created `verification/current/` as the canonical CURRENT-state surface: the state manifest,
 * the capability register, the beta register and the machine-derived script registry. Every one of
 * those is meant to change when the product changes — §264 updated three of them — and the script
 * registry is regenerated by a tool on purpose.
 *
 * The first version of this guard treated them like frozen package members and failed §264 for
 * keeping the current-state surface current. That is the guard being wrong, not the section: an
 * index that cannot be updated without tripping the corruption alarm is an index that will go
 * stale, which is the exact failure §263 existed to remove. Frozen evidence is what must not
 * change; the pointer to it is what must.
 */
...
/** The §263 living current-state surface. Mutable by design; see the header. */
const LIVING_INDEX_PREFIX = 'verification/current/';
...
export interface EvidenceIntegrityResult {
  ...
  readonly livingIndexChanged: number;
...
export function checkEvidenceIntegrity(): EvidenceIntegrityResult {
  ...
  for (const line of status) {
    ...
    if (path.startsWith(LIVING_INDEX_PREFIX)) { livingIndexChanged += 1; continue; }
...
function main(): void {
  ...
  console.log(`  living index updated       ${result.livingIndexChanged} (verification/current/, by design)`);
