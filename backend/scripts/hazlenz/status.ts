...
interface State {
  ...
  integration: {
    ...
    route: { method: string; path: string; throttle: string };
...
function main(): void {
  ...
  console.log(`  route              ${state.integration.route.method} ${state.integration.route.path}`);
  console.log(`  throttle           ${state.integration.route.throttle}`);
  console.log(`  states reachable   ${state.integration.statesReachableToday.length} of 8 `
    + '(CONFIRMED and OVERRIDDEN await the confirmation action)');
