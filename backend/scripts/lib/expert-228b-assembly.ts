/**
 * §228B ASSEMBLY. The one assembly path for the frozen §228A cohort, both legs.
 *
 * It is a transcription of `expert-221-assembly.ts` onto the §228A cases: the same modules, the
 * same order, the same payload construction. Nothing about the frozen instrument is reinterpreted
 * here — this file only builds the bytes the executor transmits, and the executor refuses to send
 * anything whose digest does not match what §228A pinned.
 */
...
import { INTEGRATED_CASES_228A, type IntegratedCase228A } from './expert-228a-integrated-instrument';

export const ASSEMBLY_228B_VERSION = 'hazlenz.expert.228b.assembly.v1' as const;

const sha = (s: string): string => createHash('sha256').update(s).digest('hex');

export function analysisIdFor228A(caseId: string): string { return `ANL-228A-${caseId}`; }
export function observationSourceIdFor228A(caseId: string): string { return `OBS-228A-${caseId}`; }

export interface AssembledFirstPass228B {
...
export function assembleFirstPass228B(): readonly AssembledFirstPass228B[] {
  return INTEGRATED_CASES_228A.map((x, i) => {
...
export interface AssembledVerifier228B {
  readonly caseId: string;
...
export interface VerifierAssemblyResult228B {
  readonly built: readonly AssembledVerifier228B[];
 ...
 *
 * The §218 contract reviews ONE target per request. §228A books exactly one verifier call per
 * verifier-bearing case, so where a case admits more than one declaration the first is the target
 * and the rest are siblings the verifier must not wander into. That is the C4 scope-containment
 * observation and it is the frozen design, not a choice made here.
 */
export function assembleVerifierFor228B(
  caseId: string, firstPassOutput: Record<string, unknown>,
): VerifierAssemblyResult228B {
  const c = INTEGRATED_CASES_228A.find(x => x.caseId === caseId) as IntegratedCase228A;
  const observationSourceId = observationSourceIdFor228A(caseId);
  ...
  const built: AssembledVerifier228B[] = [];
