/**
 * §236 — EXECUTE the frozen stabilized-contract confirmation cohort.
 *
 * NINE provider calls, exactly. SINGLE ARM. FIRST-PASS LEG ONLY. Hard ceiling USD 1.40.
 * ZERO database operations. No commit, no push, no tag, no deploy.
 *
 * NO RETRIES OF ANY KIND. The authorization books nine calls with no retry allowance, so a
 * transport or HTTP failure that yields no adjudicable response TERMINATES the run as EXECUTION
 * INCOMPLETE and returns for authorization. It is not absorbed, not retried, and coverage is never
 * reduced to fit.
 *
 * The executor REFUSES TO TRANSMIT unless the frozen package digest, the frozen instrument digest,
 * the §233 implementation digest, the §235 stabilization digest and the 29-module protected
 * composite identity are exactly the ones frozen before the spend.
 */
...
import {
  CONFIRMATION_CASES_236, instrumentDigest236, FROZEN_EXECUTION_CONFIGURATION_236,
} from './lib/expert-236-confirmation-instrument';
import {
  assembleFirstPass236, ASSEMBLY_236_VERSION, type AssembledFirstPass236,
} from './lib/expert-236-assembly';
...
const EXECUTOR_VERSION = 'hazlenz.expert.236.confirmation-execution.v1';

const AUTHORIZED_PACKAGE_DIGEST =
  'cabfc9e253f4ac67859184f5c26f9b782eac0936bdcfd6b58f089650c3a75f98';
const AUTHORIZED_INSTRUMENT_DIGEST =
  '673d72d5f2873848317ea749464ec28de3cca347204fa735d9d85edda71a5ae4';
const AUTHORIZED_IMPLEMENTATION_DIGEST_233 =
  '5517337ded68b7b8901dcb588355f98b4bdbf2dfe4ae54a4af0266af1d4ce5af';
const AUTHORIZED_STABILIZATION_DIGEST_235 =
...
const MAX_CALLS = 9;
const SPEND_CEILING_USD = 1.40;
const FIRST_PASS_MAX_TOKENS = FROZEN_EXECUTION_CONFIGURATION_236.firstPassMaxTokens;
const WORST_CALL_USD = FROZEN_EXECUTION_CONFIGURATION_236.guardWorstCaseCallUsd;
...
const abort = (m: string): never => { throw new Error(`§236 ABORT BEFORE PROVIDER EXECUTION: ${m}`); };

// ================================================================ pre-spend identity

const FILES = ['SECTION-236-INSTRUMENT.json', 'SECTION-236-TRUTH-CONTRACT.json',
  'SECTION-236-TRUTH-PREFLIGHT.json', 'SECTION-236-SCORING-RULES.json',
  'SECTION-236-FROZEN-PROTOCOL.json'];
const docs = FILES.map(f => [f, JSON.parse(readFileSync(join(EVID, f), 'utf8'))] as const);
const packageDigest = sha(docs.map(([n, d]) => `${n}\n${JSON.stringify(d)}`).join('\n'));
const protocol = docs.find(([n]) => n === 'SECTION-236-FROZEN-PROTOCOL.json')![1] as any;
const preflightDoc = docs.find(([n]) => n === 'SECTION-236-TRUTH-PREFLIGHT.json')![1] as any;
const truthDoc = docs.find(([n]) => n === 'SECTION-236-TRUTH-CONTRACT.json')![1] as any;

if (preflightDoc.result !== 'PASS') abort('the frozen truth preflight did not pass');
if (truthDoc.perCase.length !== 9) abort('the frozen truth contract is not nine cases');
if (protocol.instrumentDigest !== AUTHORIZED_INSTRUMENT_DIGEST) abort('instrument digest mismatch');
const liveInstrument = instrumentDigest236();
...
if (implementationDigest233 !== AUTHORIZED_IMPLEMENTATION_DIGEST_233) {
  abort(`the §233 implementation changed since the freeze (${implementationDigest233})`);
}
...
if (stabilizationDigest235 !== AUTHORIZED_STABILIZATION_DIGEST_235) {
  abort(`the §235 stabilization changed since the freeze (${stabilizationDigest235})`);
}

const PROT = join(EVID, 'PROTECTED-IDENTITIES-236-AT-EXECUTION.json');
...
const firstPass: AssembledFirstPass236[] = [...assembleFirstPass236()];
if (firstPass.length !== 9) abort('assembly did not produce nine first-pass calls');
for (const x of firstPass) {
  const c = CONFIRMATION_CASES_236.find(y => y.caseId === x.caseId)!;
...
const preSpend = {
  artifact: 'SECTION-236-PRE-SPEND-IDENTITY',
  executorVersion: EXECUTOR_VERSION, assemblyVersion: ASSEMBLY_236_VERSION,
  ...
  section233Unchanged: true, section235Unchanged: true,
  moduleDigests233, moduleDigests235,
  protectedIdentity: {
  ...
  frozenExecutionConfiguration: {
    ...FROZEN_EXECUTION_CONFIGURATION_236,
...
writeFileSync(join(EVID, 'SECTION-236-PRE-SPEND-IDENTITY.json'),
...
if (process.argv[2] === '--identity-only') {
  ...
  console.log(`impl §233         ${implementationDigest233}`);
  console.log(`stabilization §235 ${stabilizationDigest235}`);
...
const LEDGER = join(EVID, 'CALL-LEDGER-236.jsonl');
...
async function drawFirstPass(x: AssembledFirstPass236): Promise<CallOutcome> {
  ...
  write('RAW-236-FIRST-PASS.jsonl', {
    callIndex: callsMade, recordKind: 'RAW_FIRST_PASS_236', callKind: 'PRIMARY',
    executorVersion: EXECUTOR_VERSION, assemblyVersion: ASSEMBLY_236_VERSION,
    frozenInstrumentDigest: AUTHORIZED_INSTRUMENT_DIGEST,
    implementationDigest233: AUTHORIZED_IMPLEMENTATION_DIGEST_233,
    stabilizationDigest235: AUTHORIZED_STABILIZATION_DIGEST_235,
    caseId: x.caseId, ordinal: x.ordinal, contractVersion: x.contractVersion,
    analysisId: x.analysisId, observationSourceId: x.observationSourceId,
  ...
  const cess = typeof p === 'object' && p !== null
    && Array.isArray(p.establishedConditionsRequiringCessation)
    ? String(p.establishedConditionsRequiringCessation.length) : '-';
  console.log(`  ${String(callsMade).padStart(2)} ${x.caseId.padEnd(3)} `
    + `${out.failureClass.padEnd(17)} posture=${posture.padEnd(25)} cess=${cess.padEnd(3)} `
...
void (async () => {
  console.log(`§236 STABILIZED CONFIRMATION — instrument ${AUTHORIZED_INSTRUMENT_DIGEST.slice(0, 16)}…`);
  console.log(`§233 ${AUTHORIZED_IMPLEMENTATION_DIGEST_233.slice(0, 12)}… · §235 `
    + `${AUTHORIZED_STABILIZATION_DIGEST_235.slice(0, 12)}… · composite `
    + `${AUTHORIZED_COMPOSITE_IDENTITY.slice(0, 12)}…`);
  console.log(`9 calls, single arm, first-pass only, no retries, ceiling USD ${SPEND_CEILING_USD}\n`);
  ...
  for (const x of firstPass) {
    ...
    // NO RETRY. A transport failure terminates the run for a product-owner decision.
  ...
  const summary = {
    artifact: 'SECTION-236-EXECUTION-SUMMARY',
    executorVersion: EXECUTOR_VERSION, assemblyVersion: ASSEMBLY_236_VERSION,
    frozenInstrumentDigest: AUTHORIZED_INSTRUMENT_DIGEST,
    frozenPackageDigest: packageDigest,
    implementationDigest233: AUTHORIZED_IMPLEMENTATION_DIGEST_233,
    ...
    complete: callsMade === 9 && transportFailures.length === 0,
  ...
  writeFileSync(join(EVID, 'SECTION-236-EXECUTION-SUMMARY.json'),
