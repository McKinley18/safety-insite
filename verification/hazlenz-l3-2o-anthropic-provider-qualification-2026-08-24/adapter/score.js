/** L3-2o scorer. Metric definitions are BYTE-IDENTICAL to L3-2n/adapter/score.js; only the row
 *  set differs. D-58 is respected: MODEL tier and VALIDATED tier are never reported as one number. */
const fs=require('fs');
function score(f){
...
const sets=[
 ['claude-sonnet-5   A',process.argv[2]+'/results/S5-SHIPPED_A.json'],
 ['claude-sonnet-5   B',process.argv[2]+'/results/S5-SHIPPED_B.json'],
 ['gemini-3.7-flash A *',B+'hazlenz-l3-2n-provider-qualification-2026-08-24/results/F37-SHIPPED_A.json'],
 ['gemini-3.6-flash A *',B+'hazlenz-l3-2n-provider-qualification-2026-08-24/results/F36-SHIPPED_A.json'],
...
console.log('');
for(const [k,f] of sets.slice(0,2)){ let t; try{t=score(f);}catch{continue;}
  console.log(k+' rejections: '+(t.rids.join(',')||'(none)')+'   errors: '+t.errs); }
// Reproducibility: row-level diff between the two isolated processes.
try{
  const A=JSON.parse(fs.readFileSync(process.argv[2]+'/results/S5-SHIPPED_A.json','utf8')).rows;
  const Bq=JSON.parse(fs.readFileSync(process.argv[2]+'/results/S5-SHIPPED_B.json','utf8')).rows;
  const key=r=>JSON.stringify([r.outcome,r.candidateCount,r.modelStates,r.modelAssertsActive,
    r.validationState,r.validatedAssertsActive,r.candidateBorneClarification,r.validatedProposalLevelClarification]);
  const diff=A.filter((r,i)=>key(r)!==key(Bq[i])).map(r=>r.scenarioId);
  console.log(`\nNOISE FLOOR (two isolated processes): ${diff.length}/${A.length} rows differ  ${diff.join(',')}`);
}catch(e){console.log('\nnoise floor: unavailable');}
