...
const base = process.env.HAZLENZ_BASE_URL || 'http://127.0.0.1:4001';
...
if (loginRes.status !== 201 || !loginRes.body.token) throw new Error(`Login failed: ${loginRes.status} ${JSON.stringify(loginRes.body)}`);
const token = loginRes.body.token;
console.log('Login successful. Running', manifest.rows.length, 'cases, ~2.2s pace...');
...
const summary = { version: 'FAMILY_MATRIX_SCORE_V4_AUTHORITATIVE_REMEDIATION_20260816', rows: scored.length, byKind: {} };
for (const kind of ['positive', 'negative', 'ambiguity', 'safe']) {
  ...
  summary.byKind[kind] = { total: group.length, pass, fail: group.length - pass, outcomes: group.reduce((a, r) => { a[r.outcome] = (a[r.outcome] || 0) + 1; return a; }, {}) };
}
const totalPass = scored.filter((r) => r.outcome === 'PASS').length;
summary.totalPass = totalPass;
summary.totalRows = scored.length;
fs.writeFileSync(new URL('./V4_228_RESULT.json', import.meta.url), JSON.stringify({ summary, scored }, null, 2));
console.log('DONE:', JSON.stringify(summary.byKind, null, 2), 'TOTAL PASS:', totalPass, '/', scored.length);
