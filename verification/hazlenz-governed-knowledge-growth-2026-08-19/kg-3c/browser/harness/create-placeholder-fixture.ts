...
const OBSERVATIONS = [
  {
    label: 'general-industry egress (target: placeholder-provenance 1910.36)',
...
async function main() {
  ...
  const inspection = await api('/inspections', {
    ...
    body: JSON.stringify({ siteId: site.id, title: 'KG-3C placeholder provenance verification', regulatoryContext: 'osha-general-industry' }),
  }, 201);
  ...
  const targetCitation = process.env.FIXTURE_APPROVE_CITATION || '';
  if (!targetCitation) {
    console.log('\nNo approval requested — placeholder fixture leaves the corpus untouched.');
    console.log('\n' + JSON.stringify({ fixtureReady: true, email, password, inspectionId: inspection.id, siteId: site.id, findingsPatched: 0 }));
