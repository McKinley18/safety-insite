...
if (loginRes.status !== 201 && loginRes.status !== 200 || !(loginRes.body.token || loginRes.body.accessToken)) throw new Error(`Login failed: ${loginRes.status} ${JSON.stringify(loginRes.body)}`);
...
for (const kind of ['positive', 'negative', 'ambiguity', 'safe']) {
  ...
  summary.byKind[kind] = { total: group.length, passed };
}
summary.totalPassed = scored.filter((r) => r.pass).length;
summary.totalRows = scored.length;
fs.writeFileSync(new URL('./P1_228_PACED_RESULT.json', import.meta.url), JSON.stringify({ summary, scored }, null, 2));
